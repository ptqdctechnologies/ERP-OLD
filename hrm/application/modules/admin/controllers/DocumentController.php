...
class Admin_DocumentController extends Zend_Controller_Action
{
    ...
    public function deletetransAction()
    {
        ...
        {
            case 'PO':
                    $sql = "SELECT trano FROM procurement_rpid WHERE po_no = '$trano' GROUP BY trano;";
                ...
                break;
            case 'PR':
                    $sql = "SELECT trano,po_no FROM procurement_rpid WHERE pr_no = '$trano' GROUP BY trano;";
                    $fetch = $this->db->query($sql);
                    $hasil = $fetch->fetchAll();
                    if ($hasil)
                    {
...
//                    $return1[$k1][0] = $this->filterColumn($v);
            ...
            if ($fetch['action'] == 'UPDATE' || $fetch['action'] == 'CANCEL' || $fetch['action'] == 'REVISI')
            {
