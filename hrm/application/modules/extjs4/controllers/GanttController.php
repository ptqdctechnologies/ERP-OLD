...
//                $totalDays = intval($diff->format('%a'));
    ...
    public function scurvecostAction()
    {
        ...
        if ($cek)
        {
            $crons = $cronBudget->fetchRow("prj_kode = '$prjKode' AND sit_kode = '$sitKode'");
            $works = $this->ganttd->fetchAll("prj_kode = '$prjKode' AND sit_kode = '$sitKode'");
            if ($works)
            {
                $count = $works->count();
                $field = array();
    ...
    public function getscurvesitelistAction()
    {
        ...
        if ($siteKodeSearch)
        {
            $search = " AND sit_kode like '%$siteKodeSearch%' ";
        }
