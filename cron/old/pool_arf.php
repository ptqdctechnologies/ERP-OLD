...
$data['posts'] = $db->query('SELECT trano, (totalA -(totalB+totalC)) AS total, val_kode, tgl_last_asf, prj_kode, sit_kode, prj_nama, sit_nama, workid, workname, kode_brg, nama_brg FROM abc where (totalA -(totalB+totalC)) > 0 ORDER BY tgl_last_asf DESC');
