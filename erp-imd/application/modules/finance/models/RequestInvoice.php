...
class Finance_Models_RequestInvoice extends Zend_Db_Table_Abstract {
    ...
    public function viewallrequest($offset = 0, $limit = 0, $dir = 'DESC', $sort = 'trano', $search, $all = true, $textsearch = '', $check, $checkInvoiced) {
        ...
        if ($all) {
            if ($check != '')
                $query = "SELECT SQL_CALC_FOUND_ROWS a.trano,a.tgl,a.cus_kode,a.prj_kode,a.sit_kode,a.prj_nama,a.sit_nama,(COALESCE(a.total,0)) as total, a.invoice_detail
                    FROM " . $this->_name . " a
                ...
                $query = "SELECT SQL_CALC_FOUND_ROWS trano,tgl,cus_kode,prj_kode,sit_kode,prj_nama,sit_nama,(COALESCE(total,0)) as total, invoice_detail FROM " . $this->_name . " $where
                    ORDER BY $sort $dir LIMIT $offset,$limit";
        } else
            $query = "SELECT *,c.cus_nama,(SELECT (SUM(coalesce(a.total,0))-SUM(coalesce(b.total_cancel,0))) FROM finance_invoiced a 
                        left join finance_invoice_ci b on a.trano = b.invoice_no WHERE a.riv_no = '$textsearch' GROUP BY a.riv_no) AS totalInvoice FROM  {$this->_name} r INNER JOIN master_customer c ON (c.cus_kode=r.cus_kode) $where";
...
//                    {
                    if ($checkInvoiced != '') {
                        ...
                        $fetch2 = $fetch2->fetch();
                        $fetchCancel = $this->db->query("SELECT SUM(total_cancel) AS totalCancel FROM finance_invoice_ci WHERE riv_no = '$tranos' GROUP BY riv_no ");
                        $fetchCancel = $fetchCancel->fetch();
                        if ($fetch2) {
                            if ($fetch2['totalInvoice'] == '') {
                                $v['invoiced'] = false;
                                $v['totalInvoice'] = 0;
                            } else {
                                $v['invoiced'] = ($fetchCancel ? false : true);
                                $v['totalInvoice'] = ($fetchCancel ? $fetch2['totalInvoice'] - $fetchCancel['totalCancel'] : $fetch2['totalInvoice']);
                            }
                        }
                    }
                    $data['data'][] = $v;
...
//            $data['data']['uid_request'] = $account['displayname'][0] ;
            $data['data']['uid_request'] = QDC_User_Ldap::factory(array("uid" => $data['data']['uid']))->getName();
            ...
            $co = new Default_Models_MasterBoq2();
            $prjKode = $data['data']['prj_kode'];
            $sitKode = $data['data']['sit_kode'];
            $c = $co->fetchRow("prj_kode = '$prjKode' AND sit_kode = '$sitKode'");
            if ($c) {
                $c = $c->toArray();
                $data['data']['top'] = $c['top'];
            }
