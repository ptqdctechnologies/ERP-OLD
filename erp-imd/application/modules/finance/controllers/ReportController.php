...
//        $this->getResponse()->setBody($json);
    ...
    public function isPartial($trano) {
        $sql = "SELECT IF(totalsequence!='1','1','0')As exist  FROM imderpdb.procurement_brfh WHERE trano='$trano'";
    ...
    public function getstorepaymentAction() {
        ...
        if ($trano != '') {$search1[] = "d.trano = '$trano'";$search2[] = "ref_number = '$trano'";$search3[] = "doc_trano = '$trano'";$search4[] = "bp.trano_ref = '$trano'";$search5[] = "rem_no = '$trano'";$search6[] = "bp.trano = '$trano'";}
        ...
        if($trano=='' && $prj_kode=='' && $sit_kode=='' && $supplier==''){return;}
        ...
        if($type=='ARF')
        {
            $query = "DROP TEMPORARY TABLE IF EXISTS `bpv`;
                    ...
                    FROM imderpdb.finance_payment_voucher
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `payment`;
                    ...
                    FROM imderpdb.finance_payment_arf
                    $searchpayment 
                    GROUP BY doc_trano;";
            $this->db->query($query);
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `arf`;
                        ...
                        FROM imderpdb.procurement_arfd d
        ...
        if($type=='BRF')
        {
            if ($trano == '' && $prj_kode == '' && $sit_kode == '') {
                    $searchbrfp = "bp.trano_ref IN (SELECT ref_number FROM brf ) ";
            }
            $query = "DROP TEMPORARY TABLE IF EXISTS `brf`;
                        ...
                        FROM imderpdb.procurement_brfd d
            ...
            foreach ($datas as $key => $val) {
                ...
                if ($this->isPartial($tranos) == "0") {
                    $query = "DROP TEMPORARY TABLE IF EXISTS `brf`;
                        ...
                        SELECT d.trano as ref_number, d.val_kode, d.prj_kode, d.sit_kode, SUM(COALESCE(d.qty*d.harga,0)) AS trano_value
                        FROM imderpdb.procurement_brfd d
                        WHERE d.trano = '$tranos' AND d.deleted=0
                        GROUP BY d.trano;";
                    $this->db->query($query);
                    ...
                    $query = "DROP TEMPORARY TABLE IF EXISTS `bpv`;
                        ...
                        FROM imderpdb.finance_payment_voucher pv
                        INNER JOIN imderpdb.procurement_brfd bp ON (bp.trano = pv.ref_number AND bp.deleted=0 AND bp.trano='$tranos')
                    ...
                    $query = "DROP TEMPORARY TABLE IF EXISTS `payment`;
                        ...
                        FROM imderpdb.finance_payment_arf parf
                        INNER JOIN imderpdb.procurement_brfd bp ON (bp.trano = parf.doc_trano AND bp.deleted=0 AND bp.trano='$tranos')
                        GROUP BY doc_trano;";
                    $this->db->query($query);
                        
                    $query = "SELECT brf.ref_number,brf.val_kode, COALESCE(bpv.total_bpv,0) AS total_bpv, COALESCE(payment.total_payment,0) AS total_payment,(COALESCE(bpv.total_bpv,0)-COALESCE(payment.total_payment,0)) as balance,brf.trano_value,brf.prj_kode,brf.sit_kode FROM brf
                        LEFT JOIN bpv ON (brf.ref_number = bpv.ref_number)
                        LEFT JOIN payment ON (brf.ref_number = payment.doc_trano); $limit_query";
                    $query2 = "SELECT brf.ref_number,brf.val_kode, COALESCE(bpv.total_bpv,0) AS total_bpv, COALESCE(payment.total_payment,0) AS total_payment,(COALESCE(bpv.total_bpv,0)-COALESCE(payment.total_payment,0)) as balance,brf.trano_value,brf.prj_kode,brf.sit_kode FROM brf
                            LEFT JOIN bpv ON (brf.ref_number = bpv.ref_number)
                    ...
                    $query = "DROP TEMPORARY TABLE IF EXISTS `bpv`;
                        ...
                        FROM imderpdb.finance_payment_voucher pv
                        INNER JOIN imderpdb.procurement_brfd_payment bp ON (bp.trano = pv.ref_number AND bp.deleted=0 AND bp.trano_ref='$tranos')
                    ...
                    $query = "DROP TEMPORARY TABLE IF EXISTS `payment`;
                        ...
                        FROM imderpdb.finance_payment_arf parf
                        INNER JOIN imderpdb.procurement_brfd_payment bp ON (bp.trano = parf.doc_trano AND bp.deleted=0 AND bp.trano_ref='$tranos')
                        GROUP BY doc_trano;";
                    $this->db->query($query);
        ...
        if($type=='RPI')
        {
            $query = "DROP TEMPORARY TABLE IF EXISTS `rpi`;
                        ...
                        INNER JOIN imderpdb.procurement_rpih h ON (h.trano =d.trano)
                        $searchmain AND d.deleted=0
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `payment`;
                    CREATE TEMPORARY TABLE `payment`
                    SELECT doc_trano, SUM(COALESCE(total_bayar,0)) AS total_payment
                    FROM imderpdb.finance_payment_rpi
        ...
        if($type=='REM')
        {
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `rem`;
                        ...
                        SELECT d.trano AS ref_number,d.val_kode, d.prj_kode, d.sit_kode,SUM(COALESCE(d.qty*d.harga,0)) AS trano_value 
                        FROM imderpdb.procurement_reimbursementd d
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `payment`;
                    ...
                    FROM imderpdb.finance_payment_reimbursement
                    $searchpaymentrem 
                    GROUP BY rem_no;";
            $this->db->query($query);
        ...
        if($type=='PPNREM')
        {
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `ppn`;
                        ...
                        FROM imderpdb.finance_ppn_reimbursementh AS d
                        $searchmain
                        GROUP BY d.trano;";
        ...
        if ($type == 'RPI') {
           ...
           $this->db->query($rpi_bpv);   
            ...
            $query = "SELECT pv.ref_number AS ref_number,pv.prj_kode AS prj_kode,pv.sit_kode AS sit_kode,arf.total AS trano_value,pv.total AS total_bpv, COALESCE(parf.total_bayar,0) AS total_payment  
                        FROM imderpdb.finance_payment_voucher pv 
                        LEFT JOIN imderpdb.procurement_arfh arf ON (arf.trano=pv.ref_number)
                        LEFT JOIN imderpdb.finance_payment_arf parf ON (parf.voc_trano = pv.trano) $searchdunk";
    ...
    public function viewRemAgingAction() {
        ...
        if ($cus!=''){
             $select = "DROP TEMPORARY TABLE IF EXISTS  rem;
                    ...
                    FROM imderpdb.finance_payment_reimbursement a
                    LEFT JOIN imderpdb.procurement_reimbursementh b ON (a.rem_no = b.trano)
                    LEFT JOIN imderpdb.finance_nd_reimbursement c ON (a.trano = c.prem_no)
...
//                $days = $this->utility->dates_diff($tgl,date("Y-m-d"));
    ...
    public function viewInvoiceAgingOcAction() {
        ...
        $query="DROP TEMPORARY TABLE IF EXISTS invh;
                ...
                FROM imderpdb.finance_invoice WHERE cus_kode !='MIT' GROUP BY trano";
        $this->db->query($query);
        ...
        $query="DROP TEMPORARY TABLE IF EXISTS invd;
                ...
                FROM imderpdb.finance_invoiced WHERE cus_kode !='MIT' GROUP BY trano";
        $this->db->query($query);
    ...
    public function viewDetailPaymentReportAction(){
        ...
        if($type=='ARF')
        {
           $query=" SELECT trano, tgl,val_kode, prj_kode, sit_kode,SUM(COALESCE(qty*harga,0)) AS value
            ...
            $query="SELECT trano,tgl,val_kode, COALESCE(SUM(total_bayar),0) AS total_payment
                    FROM imderpdb.finance_payment_arf WHERE doc_trano='$trano' GROUP BY trano";
            $fetch = $this->db->query($query);
        ...
        {           
            ...
            $query = "SELECT pv.trano, pv.valuta, COALESCE(pv.total,0) AS total
                    FROM imderpdb.finance_payment_voucher pv
                    INNER JOIN imderpdb.procurement_brfd bp ON (bp.trano = pv.ref_number AND bp.deleted=0 AND bp.trano='$trano')
            ...
            $query = "SELECT parf.trano,parf.tgl,parf.val_kode, COALESCE(SUM(parf.total_bayar),0) AS total_payment
                    FROM imderpdb.finance_payment_arf parf
                    INNER JOIN imderpdb.procurement_brfd bp ON (bp.trano = parf.doc_trano AND bp.deleted=0 AND  bp.trano='$trano') GROUP BY parf.trano ";
            $fetch = $this->db->query($query);
            ...
            $query = "SELECT pv.trano, pv.valuta, COALESCE(pv.total,0) AS total
                    FROM imderpdb.finance_payment_voucher pv
                    INNER JOIN imderpdb.procurement_brfd_payment bp ON (bp.trano = pv.ref_number AND bp.deleted=0 AND bp.trano_ref='$trano')
                    WHERE pv.status_bpv_wht=0 AND pv.deleted=0";
            ...
            $query="SELECT parf.trano,parf.tgl,parf.val_kode, COALESCE(SUM(parf.total_bayar),0) AS total_payment
                    FROM imderpdb.finance_payment_arf parf
                    INNER JOIN imderpdb.procurement_brfd_payment bp ON (bp.trano = parf.doc_trano AND bp.deleted=0 AND  bp.trano_ref='$trano') GROUP BY parf.trano ";
            $fetch = $this->db->query($query);
            }
        ...
        if($type=='RPI')
        {
            ...
            $query="SELECT trano,tgl,val_kode, COALESCE(SUM(total_bayar),0) AS total_payment
                    FROM imderpdb.finance_payment_rpi WHERE doc_trano='$trano' GROUP BY trano";
            $fetch = $this->db->query($query);
            if ($fetch) {
                $fetchpayment = $fetch->fetchAll();
            }
        ...
        if($type=='REM')
        {
            ...
            $query="SELECT trano,tgl,val_kode, COALESCE(SUM(total),0) AS total_payment
                    FROM imderpdb.finance_payment_reimbursement WHERE rem_no='$trano' GROUP BY trano";
            $fetch = $this->db->query($query);
            if ($fetch) {
                $fetchpayment = $fetch->fetchAll();
            }
    ...
    public function viewDetailPaymentReportOldAction() {
        ...
        if ($type == 'RPI') {
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `brfku`;
                        CREATE TEMPORARY TABLE `brfku`
                        SELECT tgl, trano, SUM(total) AS total FROM imderpdb.procurement_brfd WHERE trano='$trano'";
            $this->db->query($query);
    ...
    public function getstorepaymentfinaldetailAction() {
        ...
        if($type=='BRF')
        {
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `bpv`;
                    ...
                    SELECT bp.trano_ref AS ref_number, SUM(COALESCE(pv.total,0)) AS total_bpv
                    FROM imderpdb.finance_payment_voucher pv
                    INNER JOIN imderpdb.procurement_brfd_payment bp ON (bp.trano = pv.ref_number AND bp.deleted=0 AND $searchbrfp )
                    WHERE pv.status_bpv_wht=0 AND pv.deleted=0
                    GROUP BY pv.ref_number;";
            $this->db->query($query);
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `payment`;
                    ...
                    SELECT bp.trano_ref AS doc_trano, SUM(COALESCE(parf.total_bayar,0)) AS total_payment
                    FROM imderpdb.finance_payment_arf parf
                    INNER JOIN imderpdb.procurement_brfd_payment bp ON (bp.trano = parf.doc_trano AND bp.deleted=0 AND $searchbrfp)
                    GROUP BY doc_trano;";
            $this->db->query($query);
 ...
 $fetch = $this->db->query($query);
        ...
        if($type=='REM')
        {
            ...
            $query = "DROP TEMPORARY TABLE IF EXISTS `rem`;
                        ...
                        END as approve
                        FROM imderpdb.procurement_reimbursementd d
                        LEFT JOIN imderpdb.procurement_reimbursementh h ON (d.trano=h.trano)
