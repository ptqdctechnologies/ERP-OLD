...
class Default_Models_Budget extends Zend_Db_Table_Abstract {
    ...
    public function getBoq3ByOne($prjKode = '', $sitKode = '', $workid = '', $kodeBrg = '', $extraQuery = '') {
        ...
        $sql = "SELECT rateidr FROM imderpdb.transengineer_boq3h WHERE prj_kode = '$prjKode' AND sit_kode = '$sitKode';";
        ...
        $sql = "DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_ori;
                CREATE TEMPORARY TABLE imderpdb.boq3_ori
                    SELECT * FROM (SELECT a.trano,
                    (IF(a.workid IN ($workidMsc),'XX',a.kode_brg)) AS kode_brg,
                    (IF(a.workid IN ($workidMsc), 'Others', (SELECT nama_brg FROM imderpdb.master_barang_project_2009 WHERE kode_brg=a.kode_brg LIMIT 0,1)))as nama_brg,
                    a.workid,
					(SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = a.workid) as workname,
                    a.val_kode,
                    a.qty,
                    a.rateidr,
                    a.cfs_kode,
                    ...
                    FROM imderpdb.transengineer_boq3d a
                    WHERE
                        a.prj_kode = '$prjKode'
                    and
                        a.sit_kode = '$sitKode'
        ...
        $this->db->query($sql);
        $sql = "DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_koreksi ;
                 CREATE TEMPORARY TABLE imderpdb.boq3_koreksi SELECT * FROM (
                        SELECT b.trano,
                        (IF(b.workid IN ($workidMsc),'XX',b.kode_brg)) AS kode_brg,
                    	(IF(b.workid IN ($workidMsc), 'Others', (SELECT nama_brg FROM imderpdb.master_barang_project_2009 WHERE kode_brg=b.kode_brg LIMIT 0,1)))as nama_brg,
                    	b.workid,
						(SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = b.workid) as workname,
                        b.val_kode,
                        b.qty,
                        rateidr,
						b.urut,
                        ...
                        FROM imderpdb.transengineer_kboq3d b WHERE
                            b.prj_kode = '$prjKode'
                        and
                            b.sit_kode = '$sitKode'
                        and
        ...
        $this->db->query($sql);
        $sql = "DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_revisi;
                CREATE TEMPORARY TABLE imderpdb.boq3_revisi
                    SELECT
                      a.workid,
					 (SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = a.workid) as workname,
                      a.kode_brg,
                    ...
                    FROM
                      imderpdb.boq3_ori a
                    LEFT JOIN
                      imderpdb.boq3_koreksi b
                    ON
                      (a.workid = b.workid AND b.kode_brg = a.kode_brg);";
        $this->db->query($sql);
        $sql = "INSERT INTO imderpdb.boq3_revisi
                    SELECT
                      b.workid,
					  (SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = b.workid) as workname,
                      b.kode_brg,
                    ...
                    WHERE
                      a.qty IS NULL;";
        $fetch = $this->db->query($sql);
        $sql = "SELECT a.*,a.workid workid, b.hargaavg FROM imderpdb.boq3_revisi a left join imderpdb.master_barang_project_2009 b on
    ...
    public function getBoq3PmealByOne($prjKode = '', $sitKode = '', $kodeBrg = '') {
        ...
        if ($rateUSD == '')
            ...
            $rateUSD = abs($rateUSD);
        $workidMsc = implode(",", $this->workidMsc);
        $sql = "CREATE TEMPORARY TABLE imderpdb.boq3_ori
                    SELECT * FROM (
                    SELECT a.trano,
                    (IF(a.workid IN ($workidMsc),'XX',a.kode_brg)) AS kode_brg,
                    (IF(a.workid IN ($workidMsc), 'Others', (SELECT nama_brg FROM imderpdb.master_barang_project_2009 WHERE kode_brg=a.kode_brg LIMIT 0,1)))as nama_brg,
        ...
        $this->db->query($sql);
        $sql = "CREATE TEMPORARY TABLE imderpdb.boq3_koreksi SELECT * FROM (
                        SELECT b.trano,
                        (IF(b.workid IN ($workidMsc),'XX',b.kode_brg)) AS kode_brg,
                    	(IF(b.workid IN ($workidMsc), 'Others', (SELECT nama_brg FROM imderpdb.master_barang_project_2009 WHERE kode_brg=b.kode_brg LIMIT 0,1)))as nama_brg,
                    	b.workid,
        ...
        $this->db->query($sql);
        $sql = "CREATE TEMPORARY TABLE imderpdb.boq3_revisi
                    SELECT
                      a.workid,
					 (SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = a.workid) as workname,
                      a.kode_brg,
        ...
        $sql = "INSERT INTO imderpdb.boq3_revisi
                    ...
                    WHERE
                      a.qty IS NULL;";
        $fetch = $this->db->query($sql);
        $sql = "SELECT * FROM imderpdb.boq3_revisi;";
        $fetch = $this->db->query($sql);
        $gTotal = $fetch->fetch();
        return $gTotal;
    }
    ...
    public function getBudgetNonProject($prjKode='', $sitKode='', $budgetId='', $budgetName='')
    {
        ...
        $sql=" SELECT id, budgetid, budgetname, val_kode,coa_kode,coa_nama,
               ...
               FROM imderpdb.transengineer_boq3dnonproject 
    ...
    public function getBudgetOverhead($prjKode = '', $sitKode = '', $budgetid = '', $action = 'current') {
        ...
        DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_overhead_sort;
        CREATE TEMPORARY TABLE imderpdb.boq3_overhead_sort
        SELECT
            id,
            trano,
            urut,
        ...
        DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_revisi_overhead_sort;
        CREATE TEMPORARY TABLE imderpdb.boq3_revisi_overhead_sort
        SELECT
            id,
            trano,
            urut,
            ...
            (CASE val_kode WHEN 'IDR' THEN (total) ElSE 0.00 END) AS totalHargaIDR,
            (CASE val_kode WHEN 'USD' THEN (total) ElSE 0.00 END) AS totalHargaUSD
            FROM imderpdb.transengineer_kboq3dnonproject
            WHERE
                prj_kode = '$prjKode'
            and
                sit_kode = '$sitKode'
        ...
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_overhead_ori ;
            CREATE TEMPORARY TABLE imderpdb.boq3_overhead_ori
                SELECT * FROM imderpdb.boq3_overhead_sort a GROUP BY a.budgetid;
        ";
        $this->db->query($sql);

        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_overhead_koreksi ;
            CREATE TEMPORARY TABLE imderpdb.boq3_overhead_koreksi
                SELECT * FROM imderpdb.boq3_revisi_overhead_sort a GROUP BY a.budgetid;
        ";
        $this->db->query($sql);

        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_overhead_revisi;
            CREATE TEMPORARY TABLE imderpdb.boq3_overhead_revisi
                SELECT
                    a.id,
                    a.prj_kode,
                    a.prj_nama,
                ...
                FROM
                    imderpdb.boq3_overhead_ori a
                LEFT JOIN
                    imderpdb.boq3_overhead_koreksi b
                ON
        ...
        switch ($action) {
            case 'current':
                $sql = "
					    SELECT
                            SQL_CALC_FOUND_ROWS *
                        FROM imderpdb.boq3_overhead_revisi;";
                $fetch = $this->db->query($sql);
    ...
    public function getBoq3($action = 'summary', $prjKode = '', $sitKode = '', $startDate = '', $endDate = '', $extraQuery = '', $afeNo = '') {
        ...
        $sql = "SELECT COALESCE(NUll,rateidr) as rateidr FROM imderpdb.transengineer_boq3h WHERE prj_kode = '$prjKode' LIMIT 1;";
        $fetch = $this->db->query($sql);
        $fetch = $fetch->fetch();
        if ($fetch)
        ...
        $sql = "
            ...
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_ori_sort;
            CREATE TEMPORARY TABLE imderpdb.boq3_ori_sort
                SELECT a.trano,
                    (IF(a.workid IN ($workidMsc),'XX',a.kode_brg)) AS kode_brg,
                    (IF(a.workid IN ($workidMsc), 'Others', (SELECT nama_brg FROM imderpdb.master_barang_project_2009 WHERE kode_brg=a.kode_brg LIMIT 0,1)))as nama_brg,
                    a.workid,
                    ...
                    (CASE val_kode WHEN 'IDR' THEN (a.harga * a.qty) ElSE 0.00 END) AS totalHargaIDR,
                    (CASE val_kode WHEN 'USD' THEN (a.harga * a.qty) ElSE 0.00 END) AS totalHargaUSD
                FROM imderpdb.transengineer_boq3d a WHERE
                    a.prj_kode = '$prjKode'
                    $whereSitKode1
                and
                    a.rev = 'N'
        ...
        $this->db->query($sql);
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_ori ;
            CREATE TEMPORARY TABLE imderpdb.boq3_ori
                SELECT * FROM imderpdb.boq3_ori_sort a GROUP BY a.workid,a.kode_brg;
        ";
        $this->db->query($sql);
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_koreksi_sort ;
            CREATE TEMPORARY TABLE imderpdb.boq3_koreksi_sort
                    SELECT b.trano,
                    (IF(b.workid IN ($workidMsc),'XX',b.kode_brg)) AS kode_brg,
                    (IF(b.workid IN ($workidMsc), 'Others', (SELECT nama_brg FROM imderpdb.master_barang_project_2009 WHERE kode_brg=b.kode_brg LIMIT 0,1)))as nama_brg,
                    b.workid,
        ...
        $this->db->query($sql);
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_koreksi ;
            CREATE TEMPORARY TABLE imderpdb.boq3_koreksi
                SELECT * FROM imderpdb.boq3_koreksi_sort a GROUP BY a.workid,a.kode_brg;
        ";
        $this->db->query($sql);
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_revisi;
            CREATE TEMPORARY TABLE imderpdb.boq3_revisi
                SELECT
                    a.workid,
			        (SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = a.workid) as workname,
        ...
        $this->db->query($sql);
        $sql = "
            INSERT INTO imderpdb.boq3_revisi
                SELECT
                    b.workid,
			        (SELECT workname FROM imderpdb.masterengineer_work WHERE  workid = b.workid) as workname,
                    b.cfs_kode,
        ...
        switch ($action) {
            ...
            case 'all-ori':
                $sql = "
                        SELECT
 	                        *
                        FROM imderpdb.boq3_ori ORDER BY workid ASC, kode_brg ASC;
                    ";
                $fetch = $this->db->query($sql);
            ...
            case 'all-current':
                if ($this->useTempTable) {
                    ...
                    $sql = "
                        SELECT
 	                        a.* , b.hargaavg
                        FROM imderpdb.boq3_revisi a left join imderpdb.master_barang_project_2009 b on(a.kode_brg=b.kode_brg) ORDER BY a.workid ASC, a.kode_brg ASC;
                    ";
    ...
    public function getBoq3PR($action = 'summary', $prjKode = '', $sitKode = '', $startDate = '', $endDate = '', $extraQuery = '', $afeNo = '') {
        ...
        $sql = "
            ...
            CREATE TEMPORARY TABLE imderpdb.boq3_ori_sort
                SELECT a.trano,
                    ...
                    (CASE val_kode WHEN 'IDR' THEN (a.harga * a.qty) ElSE 0.00 END) AS totalHargaIDR,
                    (CASE val_kode WHEN 'USD' THEN (a.harga * a.qty) ElSE 0.00 END) AS totalHargaUSD
                FROM imderpdb.transengineer_boq3d a WHERE a.stspmeal ='N' AND
                    a.prj_kode = '$prjKode'
                    $whereSitKode1
                and
                    a.rev = 'N'
        ...
        $sql = "
            ...
            CREATE TEMPORARY TABLE imderpdb.boq3_koreksi_sort
                    ...
                    (CASE val_kode WHEN 'IDR' THEN (harga * qty) ElSE 0.00 END) AS totalHargaIDR,
                    (CASE val_kode WHEN 'USD' THEN (harga * qty) ElSE 0.00 END) AS totalHargaUSD
                    FROM imderpdb.transengineer_kboq3d b WHERE (b.stspmeal ='N' OR b.stspmeal IS NULL) AND
                        b.prj_kode = '$prjKode'
                        $whereAfeNo    
                        $whereSitKode2
                        $sqlDate
...
//                    	}
    ...
    public function getBoq2AFE($prjKode = '', $sitKode = '') {
        $sql = "SELECT rateidr, COALESCE(SUM(total),0) AS totalIDR, COALESCE(SUM(totalusd),0) AS totalUSD,COALESCE(SUM(totalusd*rateidr),0) AS totalUSDinIDR 
                FROM imderpdb.transengineer_boq2h 
    ...
    public function getKBoq2AFE($prjKode = '', $sitKode = '', $finalDate = '') {
        ...
        $sql = "SELECT COALESCE(SUM(totaltambah),0) AS totalIDR, COALESCE(SUM(totaltambahusd),0) AS totalUSD , COALESCE(SUM(totaltambahusd*rateidr),0) AS totalUSDinIDR 
                FROM imderpdb.transengineer_kboq2h 
    ...
    public function getBoq3AFE($prjKode = '', $sitKode = '') {
        $sql = "SELECT rateidr, SUM(CASE val_kode WHEN 'USD' THEN qty*harga ELSE 0 END) AS totalUSD, 
                ...
                FROM imderpdb.transengineer_boq3d 
    ...
    public function getTotalAFE($afe_no) {
        $sql1 = "SELECT 
                    ...
                    FROM imderpdb.transengineer_afeds   
                    WHERE trano='" . $afe_no . "'";
        ...
        $sql2 = "SELECT 
                        ...
                        SUM(CASE val_kode WHEN 'USD' THEN qtybaru * hargabaru ELSE 0 END) AS totalBaruUSD, 
                        SUM(CASE val_kode WHEN 'USD' THEN qty*harga ELSE 0 END) AS totalAwalUSD
                    FROM imderpdb.transengineer_afed  
                    WHERE trano='" . $afe_no . "'";
    ...
    public function getTotalPrevious($prjKode = '', $sitKode = '', $afeno = '') {
        ...
        $sql = "SELECT trano FROM imderpdb.transengineer_kboq3d 
        ...
        if ($afe['trano'] != NULL) {
            ...
            $sql = "
                     DROP TEMPORARY TABLE IF EXISTS imderpdb.afe_final_approve ;
                     CREATE TEMPORARY TABLE imderpdb.afe_final_approve
                     SELECT afe_no FROM imderpdb.transengineer_kboq3d 
        ...
        $sql1 = "SELECT 
                    ...
                    FROM imderpdb.afe_final_approve AS a
                    INNER JOIN imderpdb.transengineer_afeds AS b ON (b.trano = a.afe_no);";
        ...
        $sql2 = "SELECT 
                        ...
                        SUM(CASE b.val_kode WHEN 'USD' THEN b.qty*b.harga ELSE 0 END) AS totalAwalUSD
                    FROM imderpdb.afe_final_approve AS a
                    INNER JOIN imderpdb.transengineer_afed AS b ON (b.trano = a.afe_no);";
    ...
    public function createOnGoingAFESaving($prjKode='')
    {
        ...
        $sql ="DROP TEMPORARY TABLE IF EXISTS imderpdb.OnGoingAFE;
                CREATE TEMPORARY TABLE imderpdb.OnGoingAFE
                (
        ...
        $sql1="DROP TEMPORARY TABLE IF EXISTS imderpdb.OnGoingAFE2;
                CREATE TEMPORARY TABLE imderpdb.OnGoingAFE2
                SELECT item_id, approve FROM imderpdb.workflow_trans WHERE item_type='AFE' AND prj_kode='$prjKode' ORDER BY workflow_trans_id DESC;";
        $this->db->query($sql1);  
        
        $sql2="DROP TEMPORARY TABLE IF EXISTS imderpdb.OnGoingAFE3;
               CREATE TEMPORARY TABLE imderpdb.OnGoingAFE3
               SELECT * FROM imderpdb.OnGoingAFE2 GROUP BY item_id;";
        $this->db->query($sql2);
        
        $sql3 ="INSERT INTO imderpdb.OnGoingAFE (trano,prj_kode,sit_kode,workid,kode_brg,val_kode,qty,harga)
                SELECT trano,prj_kode,sit_kode,workid,kode_brg,val_kode,qtybaru,hargabaru
                FROM imderpdb.transengineer_afeds a
                INNER JOIN imderpdb.OnGoingAFE3 b ON (b.item_id = a.trano)
                WHERE b.approve < 300 ;";
    ...
    public function createOnGoingAFE($prjKode='')
    {
        ...
        $sql3 ="INSERT INTO imderpdb.OnGoingAFE (trano,prj_kode,sit_kode,workid,kode_brg,val_kode,qty,harga)
                SELECT trano,prj_kode,sit_kode,workid,kode_brg,val_kode,qtybaru,hargabaru
                FROM imderpdb.transengineer_afed a
                INNER JOIN imderpdb.OnGoingAFE3 b ON (b.item_id = a.trano)
                WHERE b.approve < 300 ;";
              
        $this->db->query($sql3);
        
        $sql4 ="INSERT INTO imderpdb.OnGoingAFE (trano,prj_kode,sit_kode,workid,kode_brg,val_kode,qty,harga)
                SELECT trano,prj_kode,sit_kode,workid,kode_brg,val_kode,qtybaru,hargabaru
                FROM imderpdb.transengineer_afeds a
                INNER JOIN imderpdb.OnGoingAFE3 b ON (b.item_id = a.trano)
                WHERE b.approve < 300 ;";
    ...
    public function getBOQ3CurrentPerItemNonPeacemeal($prjKode = '', $sitKode = '',$valKode='',$workId='',$kodeBrg='') {
        ...
        $sql = "DROP TEMPORARY TABLE IF EXISTS imderpdb.updated_boq3;
                CREATE TEMPORARY TABLE imderpdb.updated_boq3
                ...
                FROM imderpdb.transengineer_boq3d WHERE prj_kode='$prjKode' $sit_Kode AND (stspmeal ='N' OR stspmeal IS NULL) ORDER BY id DESC;";
        
        $this->db->query($sql);

        $sql2 = "DROP TEMPORARY TABLE IF EXISTS imderpdb.group_updated_boq3;
                 CREATE TEMPORARY TABLE imderpdb.group_updated_boq3
                 SELECT id, tgl,prj_kode,prj_nama,sit_kode,sit_nama,cfs_kode,cfs_nama,kode_brg,nama_brg,workid,workname,qty,harga,val_kode,rateidr
                 FROM  imderpdb.updated_boq3 GROUP BY kode_brg,workid;";
        ...
        $sql3 = "SELECT 
                    ...
                    FROM imderpdb.group_updated_boq3 WHERE prj_kode='$prjKode' $sit_Kode $val_Kode $work_Id $kode_Brg ;";
    ...
    public function getBOQ3CurrentPerItem($prjKode = '', $sitKode = '',$valKode='') {
        ...
        $sql = "DROP TEMPORARY TABLE IF EXISTS imderpdb.updated_boq3;
                CREATE TEMPORARY TABLE imderpdb.updated_boq3
                SELECT id,tgl,prj_kode,prj_nama,sit_kode,sit_nama,cfs_kode,cfs_nama,kode_brg,nama_brg,workid,workname,qty,harga,val_kode,rateidr
                FROM imderpdb.transengineer_boq3d WHERE prj_kode='$prjKode' $sitKode  ORDER BY id DESC;";
        ...
        $sql3 = "SELECT 
                    ...
                    (CASE val_kode WHEN 'USD' THEN qty*harga ELSE 0 END) AS totalUSD,
                    (CASE val_kode WHEN 'USD' THEN qty*harga*rateidr ELSE qty * harga END) AS total
                    FROM imderpdb.group_updated_boq3 $valKode ;";
    ...
    public function getBOQ3Current($prjKode = '', $sitKode = '',$endDate = '', $afeno='') {
        ...
        if ($afeno != '' || $afeno !=null) {
            $sql0 = "SELECT trano FROM imderpdb.transengineer_kboq3d WHERE prj_kode='$prjKode' $sitKode and afe_no ='$afeno' LIMIT 0,1;";

            $fetch0 = $this->db->query($sql0);
            $kboq3 = $fetch0->fetch();
            
            $sql10 = "SELECT MIN(id) AS id FROM imderpdb.transengineer_boq3d WHERE prj_kode='$prjKode' $sitKode and tranorev ='".$kboq3['trano']."' LIMIT 0,1;";
        ...
        $sql = "DROP TEMPORARY TABLE IF EXISTS imderpdb.updated_boq3;
                ...
                FROM imderpdb.transengineer_boq3d WHERE prj_kode='$prjKode' $sitKode $date $trano ORDER BY id DESC;";
        $this->db->query($sql);

        $sql2 = "DROP TEMPORARY TABLE IF EXISTS imderpdb.group_updated_boq3;
                 CREATE TEMPORARY TABLE imderpdb.group_updated_boq3
                 SELECT tgl,prj_kode,sit_kode,kode_brg,nama_brg,workid,qty,harga,val_kode FROM  imderpdb.updated_boq3 GROUP BY kode_brg,workid;";
        ...
        $sql3 = "SELECT 
                    ...
                    FROM imderpdb.group_updated_boq3;";
    ...
    public function getBoq2($action = 'summary', $prjKode = '', $sitKode = '') {
        if ($action == 'summary-ori' || $action == 'summary-current') {
            ...
            $sql = "CREATE TEMPORARY TABLE imderpdb.boq2_ori
                    SELECT *
                    FROM
                     imderpdb.transengineer_boq2h a
                    WHERE
                      prj_kode = '$prjKode' AND sit_kode='$sitKode'
                    ORDER BY
                      sit_kode ASC";
            $fetch = $this->db->query($sql);
            $sql = "CREATE TEMPORARY TABLE imderpdb.boq2_koreksi
                    SELECT *
                    FROM
                     imderpdb.transengineer_kboq2h a
                    WHERE
                      prj_kode = '$prjKode' AND sit_kode='$sitKode'
                    ORDER BY
                      sit_kode ASC";
            $fetch = $this->db->query($sql);
            $sql = "CREATE TEMPORARY TABLE imderpdb.boq2_current
                    SELECT *
                    FROM imderpdb.boq2_ori";
            $fetch = $this->db->query($sql);
            $sql = "INSERT INTO imderpdb.boq2_current
                    SELECT *
                    FROM imderpdb.boq2_koreksi";
            $fetch = $this->db->query($sql);
            switch ($action) {
                case 'all-ori':
                    $sql = "SELECT *
...
//                    ) a;";
            $sql = "SELECT
            		...
            		SELECT
                        ...
                        SUM(CASE val_kode WHEN 'USD' THEN (harga*qty*rateidr) ElSE 0.00 END) AS totalUSD,
                        SUM(CASE val_kode WHEN 'USD' THEN (harga*qty) ElSE 0.00 END) AS totalHargaUSD
                    FROM imderpdb.procurement_mdod
                    WHERE
                            prj_kode = '$prjKode'
                        $sitKode
                        AND asalbarang='WH') a";
            ...
            $sql = "SELECT
                    ...
                    FROM(
                        SELECT
                            ...
                            SUM(CASE val_kode WHEN 'USD' THEN (harga*qty*rateidr) ElSE 0.00 END) AS totalUSD,
                            SUM(CASE val_kode WHEN 'USD' THEN (harga*qty) ElSE 0.00 END) AS totalHargaUSD
                        FROM imderpdb.procurement_rpid
                        WHERE
                            prj_kode = '$prjKode'
                            $sitKode
                            $SQLsite
    ...
    public function getBudgetProject($all = false, $prjKode = '', $sitKode = '', $detail = false, $offset = 0, $allSite = false) {
        ...
        if ($all) {
            $sql = "SELECT
                    ...
                    s.sit_nama,
                    s.stsoverhead
                FROM imderpdb.master_project p
                LEFT JOIN imderpdb.master_site s
...
//        $totalASFCancelHargaUSD = $ASFCancel['totalHargaUSD'];
            ...
            $sql = "SELECT
                    ...
                    FROM(
                        SELECT
                            ...
                            SUM(CASE val_kode WHEN 'USD' THEN (harga*qty*rateidr) ElSE 0.00 END) AS totalUSD,
                            SUM(CASE val_kode WHEN 'USD' THEN (harga*qty) ElSE 0.00 END) AS totalHargaUSD
                        FROM imderpdb.procurement_reimbursementd
                        WHERE
                            prj_kode = '$prjKode'
                            AND sit_kode = '$sitKode'
                        ) a ;";
    ...
    public function transferTempBOQ3($trano) {
        ...
        $sql = "SELECT * FROM imderpdb.transengineer_praboq3d WHERE trano = '$trano'";
        $fetch = $this->db->query($sql);
        if ($fetch) {
            ...
            $sql = "SELECT * FROM imderpdb.projectmanagement_gantt_task WHERE boq_no = '$trano'";
            $fetch2 = $this->db->query($sql);
            $fetch2 = $fetch2->fetch();
            if ($fetch2) {
                $sql = "UPDATE imderpdb.projectmanagement_gantt_task SET boq_no = '$newtrano' WHERE boq_no = '$trano'";
                $fetch2 = $this->db->query($sql);
                $sql = "UPDATE imderpdb.projectmanagement_gantt_taskd SET boq_no = '$newtrano' WHERE boq_no = '$trano'";
                $fetch2 = $this->db->query($sql);
                $sql = "UPDATE imderpdb.projectmanagement_gantt_dependency SET boq_no = '$newtrano' WHERE boq_no = '$trano'";
                $fetch2 = $this->db->query($sql);
            }

            $sql = "DELETE FROM imderpdb.transengineer_praboq3d WHERE trano = '$trano'";
            $fetch = $this->db->query($sql);

            $sql = "SELECT * FROM imderpdb.transengineer_praboq3h WHERE trano = '$trano'";
            $fetch = $this->db->query($sql);
            if ($fetch) {
                $hasil = $fetch->fetch();
                ...
                $sql = "DELETE FROM imderpdb.transengineer_praboq3h WHERE trano = '$trano'";
                $fetch = $this->db->query($sql);
            }
        ...
        switch ($action) {
            ...
            case 'all-current':
                $sql = "
                        SELECT
 	                        *
                        FROM imderpdb.boq3_revisi ORDER BY workid ASC, kode_brg ASC;
                    ";
                $fetch = $this->db->query($sql);
                $gTotal = $fetch->fetchAll();
                return $gTotal;
...
//                	break;
    ...
    public function getBoq2CFS($action = 'summary', $prjKode = '', $sitKode = '') {
        ...
        if ($action == 'summary-ori' || $action == 'summary-current') {
            ...
            $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq2_budget ;
            CREATE TEMPORARY TABLE imderpdb.boq2_budget
                SELECT (
            ...
            $this->db->query($sql);
            $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq2_ori ;
            CREATE TEMPORARY TABLE imderpdb.boq2_ori
                SELECT *
                FROM
                    imderpdb.transengineer_boq2h a
                WHERE
            ...
            $this->db->query($sql);
            $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq2_koreksi ;
            CREATE TEMPORARY TABLE imderpdb.boq2_koreksi
                SELECT *
                FROM
                    imderpdb.transengineer_kboq2h a
                WHERE
            ...
            $sql = "SELECT
                    ...
                    FROM(
                        SELECT
                            ...
                            SUM(CASE val_kode WHEN 'USD' THEN (harga*qty*rateidr) ElSE 0.00 END) AS totalUSD,
                            SUM(CASE val_kode WHEN 'USD' THEN (harga*qty) ElSE 0.00 END) AS totalHargaUSD
                        FROM imderpdb.finance_settled_detail
                        WHERE
                            prj_kode = '$prjKode'
                            $cfsKode
                        ) a ;";
        ...
        if ($action == 'summary') {
            $sql = "SELECT
                    ...
                    FROM(
                        ...
                        FROM imderpdb.finance_payment_rpid
                        WHERE
                            prj_kode = '$prjKode'
                            $cfsKode
                        ) a ;";
...
//                    ) a;";
    ...
    public function getBoq3All($action = 'summary', $prjKode = '', $sitKode = '', $offset = 0, $limit = 0, $old = false) {
        $sql = "SELECT COALESCE(rateidr,0) as rateidr FROM imderpdb.transengineer_boq3h WHERE prj_kode = '$prjKode' LIMIT 1;";
        $fetch = $this->db->query($sql);
        $fetch = $fetch->fetch();
        ...
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_ori ;
            CREATE TEMPORARY TABLE imderpdb.boq3_ori
                SELECT * FROM imderpdb.boq3_ori_sort a GROUP BY $where1;";
        $this->db->query($sql);
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_koreksi_sort ;
            CREATE TEMPORARY TABLE imderpdb.boq3_koreksi_sort
        ...
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_koreksi ;
            CREATE TEMPORARY TABLE imderpdb.boq3_koreksi
                SELECT * FROM imderpdb.boq3_koreksi_sort a GROUP BY $where1;";
        $this->db->query($sql);
        $sql = "
            DROP TEMPORARY TABLE IF EXISTS imderpdb.boq3_revisi;
            CREATE TEMPORARY TABLE imderpdb.boq3_revisi
    ...
    public function getDor($action = 'summary', $prjKode = '', $sitKode = '') {
        if ($sitKode != '')
            $sitKode = " AND sit_kode = '$sitKode'";
        $sql = "
        DROP TEMPORARY TABLE IF EXISTS imderpdb.dorTemp;
        CREATE TEMPORARY TABLE imderpdb.dorTemp
        SELECT
            trano,
            qty,
    ...
    public function getDo($action = 'summary', $prjKode = '', $sitKode = '') {
        if ($sitKode != '')
            $sitKode = " AND sit_kode = '$sitKode'";
        $sql = "
        DROP TEMPORARY TABLE IF EXISTS imderpdb.doTemp;
        CREATE TEMPORARY TABLE imderpdb.doTemp
        SELECT
            qty,
            (CASE val_kode WHEN 'IDR' THEN (qty * harga) ELSE 0.00 END)as totalHargaIDR,
            (CASE val_kode WHEN 'USD' THEN (qty * harga) ELSE 0.00 END)as totalHargaUSD
        FROM
            imderpdb.procurement_whod
        WHERE
            prj_kode = '$prjKode'
            $sitKode;
        ";
        ...
        if ($action == 'summary') {
            ...
            $sql = "SELECT rateidr, DATE_FORMAT(tgl, '%d-%m-%Y %H:%i:%s') as tgl_rate
       			FROM imderpdb.finance_exchange_rate
