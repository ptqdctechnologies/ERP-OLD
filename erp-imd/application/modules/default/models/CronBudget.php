...
class Default_Models_CronBudget extends Zend_Db_Table_Abstract
{
    ...
    {
        $sql = "UPDATE imderpdb.transengineer_boq3d SET kode_brg='XX' WHERE workid IN ('1100','2100','3100','4100','5100','6100')";
        $this->db->query($sql);
    ...
    {
        $sql = "UPDATE imderpdb.transengineer_boq3d SET stspmeal='N' WHERE workid IN ('1100','2100','3100','4100','5100','6100')";
        $this->db->query($sql);
    ...
    public function setBoq3IDRrate()
    {
        ...
        $sql2 = "UPDATE imderpdb.transengineer_boq3d d
    ...
    {
        $sql = "UPDATE imderpdb.procurement_arfd SET val_kode='IDR' WHERE trano LIKE '%BRFP%';";
        $this->db->query($sql);
        
        $sql2 = "UPDATE imderpdb.procurement_arfh SET val_kode='IDR' WHERE trano LIKE '%BRFP%';";
        $this->db->query($sql2);
    }
    ...
    public function setKBoq2IDRrate()
    {
        ...
        $sql2 = "UPDATE imderpdb.transengineer_kboq2h d
                 INNER JOIN IDRrate h ON (h.tgl = d.tgl)
                 SET d.rateidr = h.rateidr
                 WHERE d.rateidr=0;";
        $this->db->query($sql2);
    ...
    {
        $sql = 'UPDATE imderpdb.procurement_whod do 
                INNER JOIN imderpdb.procurement_pointd dor ON (dor.trano = do.mdi_no AND dor.kode_brg = do.kode_brg)
    ...
    {
        $sql = 'UPDATE imderpdb.procurement_rpid rpi 
                INNER JOIN imderpdb.master_suplier ms ON (rpi.sup_kode=ms.sup_kode) 
    ...
    {
        $sql = "UPDATE imderpdb.procurement_brfd SET kode_brg='820005-0000', nama_brg='Travel & Fares/Business Trip'
    ...
    public function setApproveColumn()
    {
        ...
        $sql = "DROP TEMPORARY TABLE IF EXISTS wf;
                ...
                SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%RPI%' ORDER BY date DESC) h
                GROUP BY item_id ;";
        $this->db->query($sql);
        
        $sql1 = "UPDATE imderpdb.procurement_rpid d
        ...
        $sql2 = "DROP TEMPORARY TABLE IF EXISTS wf;
                 CREATE TEMPORARY TABLE wf
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE (item_type LIKE '%ASF%' OR item_type LIKE '%BSF%' OR item_type LIKE '%OCA%') ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql2);
        
        $sql3 = "UPDATE imderpdb.procurement_asfdd d
                 INNER JOIN wf h ON (h.item_id = d.trano)
                 SET d.approve = h.approve;";
        $this->db->query($sql3);
        
        // CANCELED ASF & BSF
        $sql4 = "UPDATE imderpdb.procurement_asfddcancel d
                 INNER JOIN wf h ON (h.item_id = d.trano)
                 SET d.approve = h.approve;";
        $this->db->query($sql4);
        ...
        $sql5 = "DROP TEMPORARY TABLE IF EXISTS wf;
                 CREATE TEMPORARY TABLE wf
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%PO%'  ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql5);
        
        //PO
        $sql6 = "UPDATE imderpdb.procurement_pod d
                 INNER JOIN wf h ON (h.item_id = d.trano)
                 SET d.approve = h.approve;";
        $this->db->query($sql6);
        ...
        $sql7 = "DROP TEMPORARY TABLE IF EXISTS wf;
                 CREATE TEMPORARY TABLE wf
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%ARF%' ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql7);
        
        //ARF
        $sql8 = "UPDATE imderpdb.procurement_arfd d
                 INNER JOIN wf h ON (h.item_id = d.trano)
                 SET d.approve = h.approve;";
        $this->db->query($sql8);
        ...
        $sql9 = "DROP TEMPORARY TABLE IF EXISTS wf;
                 CREATE TEMPORARY TABLE wf
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%BRF%'  ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql9);
        
        $sql10 = "UPDATE imderpdb.procurement_brfd d
                    INNER JOIN wf h ON (h.item_id = d.trano)
                    SET d.approve = h.approve;";
        $this->db->query($sql10);
        ...
        $sql11 = "DROP TEMPORARY TABLE IF EXISTS wf;
                 CREATE TEMPORARY TABLE wf
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%PR%' ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql11);
        
        $sql12 = "UPDATE imderpdb.procurement_prd d
                    INNER JOIN wf h ON (h.item_id = d.trano)
                    SET d.approve = h.approve;";
        $this->db->query($sql12);
        ...
        $sql11 = "DROP TEMPORARY TABLE IF EXISTS pmeal;
                 ...
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%PBOQ3%'  ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql13);
        
        $sql12 = "UPDATE imderpdb.boq_dboqpasang d
        ...
        $sql11 = "DROP TEMPORARY TABLE IF EXISTS mreturn;
                 ...
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%iLOV%'  ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql15);
        
        $sql12 = "UPDATE imderpdb.procurement_whreturnd d
        ...
        $sql11 = "DROP TEMPORARY TABLE IF EXISTS dor;
                 ...
                 SELECT * FROM (SELECT item_id,item_type,approve FROM imderpdb.workflow_trans WHERE item_type LIKE '%DOR%'  ORDER BY date DESC) h
                 GROUP BY item_id ;";
        $this->db->query($sql17);
        
        $sql12 = "UPDATE imderpdb.procurement_pointd d
