...
class ConversationController extends Zend_Controller_Action {
    ...
    public function countmyconversationAction() {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM conversation
                WHERE
                    uid_receiver = '$myUid'
                AND
                    `read` = 0
...
//            $fromLdap = $ldap->getAccount($hasil[0]['uid_sender']);
//            $name = $fromLdap['displayname'][0];
            $name = QDC_User_Ldap::factory(array("uid" => $hasil[0]['uid_sender']))->getName();
            $return['last'] = array(
                "sender" => $name,
