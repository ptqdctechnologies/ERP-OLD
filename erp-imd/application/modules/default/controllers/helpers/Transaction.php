...
Zend_Controller_Action_Helper_Abstract {
    ...
    function getArfasf($prjKode = '', $sitKode = '') {
        ...
        if ($sitKode) {
            ...
            $where2 = " AND b.sit_kode = '$sitKode'";
        }
    
        $sql = "
        DROP TEMPORARY TABLE IF EXISTS arfd;
                CREATE TEMPORARY TABLE arfd
            SELECT *, SUM(c.total) as total_arf
            FROM (
                SELECT
                a.trano,
                a.tgl,
            ...
            c GROUP BY c.trano,c.kode_brg ORDER BY c.tgl,c.trano;
        ";
        
        $this->db->query($sql);
        $sql = "
        DROP TEMPORARY TABLE IF EXISTS asfddcancel;
        ...
        SELECT
            ...
            b.qty,
            b.harga,
            b.qty * b.harga as totalasfcancel
        FROM
            procurement_asfd a
        INNER JOIN
            procurement_asfddcancel b
        ON
        ...
        $sql = "
        DROP TEMPORARY TABLE IF EXISTS asfdd ;
        ...
        SELECT * FROM (
            ...
            ORDER BY
                tgl ,
                b.trano
        ) v  GROUP BY v.trano,v.kode_brg;
        ";
        $this->db->query($sql);
            
        $sql = "
        DROP TEMPORARY TABLE IF EXISTS arfasf;
        ...
        SELECT
            ...
            a.sit_nama,
            a.kode_brg,
            b.kode_brg AS kode_brg2,
            a.nama_brg,
            a.workid,
            a.trano as arf_num,
            a.tgl as tgl_arf,
            a.requestor,
            COALESCE(b.trano,' - ') as asf_no,
            COALESCE(b.arf_no,' - ') as arf_nbr,
            COALESCE(b.tgl,' - ' ) as tgl_asf,
            a.total as total_arf,
            COALESCE(b.totalasf,0) as total_asf,
            COALESCE(datediff(b.tgl,a.tgl),' - ') as aging_arf_days,'left1' as kode
        FROM
            arfd a
        LEFT JOIN
            asfdd b
        ...
        SELECT
            ...
            a.total as total_arf,
            COALESCE(b.totalasf,0) as total_asf,
                COALESCE(datediff(b.tgl,a.tgl),' - ') as aging_arf_days,'left2' as kode
        FROM
            arfd a
        LEFT JOIN
            asfdd b
        ...
        WHERE a.kode_brg is not null AND b.kode_brg is not null;
        ";
        $this->db->query($sql);
        ...
        SELECT
            ...
            a.total as total_arf,
            COALESCE(b.totalasf,0) as total_asf,
                COALESCE(datediff(b.tgl,a.tgl),' - ') as aging_arf_days,'right1' as kode
        FROM
            arfd a
        RIGHT JOIN
            asfdd b
        ...
        WHERE a.kode_brg is null OR b.kode_brg is null;
        ";
        $this->db->query($sql);
        ...
        SELECT
            a.*,
            b.trano as asfcancel_num,
            COALESCE(b.totalasfcancel,0) as total_asfcancel,
                COALESCE(a.total_arf,0) - COALESCE(a.total_asf,0) - COALESCE(b.totalasfcancel,0) as balance
        FROM
            arfasf a
        LEFT JOIN
            asfddcancel b
