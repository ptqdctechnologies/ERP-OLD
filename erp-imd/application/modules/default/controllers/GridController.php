...
class GridController extends Zend_Controller_Action
{
        ...
        switch($listType)
            {
                ...
                break;
                case 'cfs_list':
                     $metadata = $this->db->describeTable('master_cfs_kode');
                     $columnNames = array_keys($metadata);
                     $colView = array("cfs_kode" => array("header" => "'CFS Code'" ,"width" => 100, "sortable" => "true", "dataIndex" => "'cfs_kode'"),
                                      "cfs_nama" => array("header" => "'CFS Name'" ,"width" => 200, "sortable" => "true", "dataIndex" => "'cfs_nama'")
                     );
                break;
                case 'satuan_list':
    ...
    public function geturljsonAction()
    {
        ...
        switch($listType)
        {
            ...
            break;
            case 'cfs_list':
                $retUrl = '/projectmanagement/cfs/list';
            break;
            case 'satuan_list':
                $retUrl = '/satuan/list';
            break;
            ...
            case 'cfs_list_param':
                if (isset($additionalColumnData) && isset($additionalColumnName))
                {
                    $retUrl = '/projectmanagement/cfs/listByParams/name/' . $columnName . '/data/' . $param . '/' . $additionalColumnName . '/' . $additionalColumnData;
                }
                else
                {
                    if ($noParam)
                        $retUrl = '/projectmanagement/cfs/listByParams/name/' . $columnName . '/data/';
                    else
                        $retUrl = '/projectmanagement/cfs/listByParams/name/' . $columnName . '/data/' . $param ;
                }
            break;
