...
class UserController extends Zend_Controller_Action
{
    ...
    public function listAction()
    {
        ...
        $forceReload = ($this->_getParam("force_reload") == '') ? false : true;
        $return = $user->all(null,$sort,$dir, $limit, $offset,$forceReload);
        
        $index = 0;
        $temp=array();
        $data=array();
        
        foreach($return['posts'] AS $key => $value)
        {
            if(strrpos($return['posts'][$key]['name'],'(DELETED ON LDAP)') <= 0)
            {
                
                $temp[$index]= $return['posts'][$key];
                $index++;
            }
        }
        
        $data['count']=count($temp['posts']);
        $data['posts']=$temp;
