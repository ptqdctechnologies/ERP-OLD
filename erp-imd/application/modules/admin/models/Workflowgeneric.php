<?php
class Admin_Models_Workflowgeneric extends Zend_Db_Table_Abstract
{
...
//                                        $nextName = $nextUser['displayname'][0];
                                        $nextName = QDC_User_Ldap::factory(array("uid" => $v['uid']))->getName();
                            ...
                            {
                                //User next approval is empty (role not assign)
                                $workflowName =  $nextWork['workflow_item_name'];
                                $roles = $masterRoleType->fetchRow("id=$roleID");
                                $roleName = $roles['display_name'];
                                $msgArray = array(
                                    "success" => false,
                                    "msg" => "Next person for this Workflow is not Exist, Please contact IT Support.<br><b>Error Reason : " .
                                        "Workflow Generic " . $workflowName . " " . $nextWork['prj_kode'] . " User Role : " . $roleName . " Not Found.</b>"
                                );
...
//                                        $nextName = $nextUser['displayname'][0];
                                        $nextName = QDC_User_Ldap::factory(array("uid" => $users[0]['uid']))->getName();
                                        $nextPerson['person'][] = array(
                                            "prj_kode" => $v['prj_kode'],
...
//                                            $nextName = $nextUser['displayname'][0];
                                            $nextName = QDC_User_Ldap::factory(array("uid" => $v2['uid']))->getName();
                                            $nextPerson['person'][] = array(
                                                "prj_kode" => $v['prj_kode'],
                                                "uid_next" => $v2['uid'],
                                                "uid_prev" => $prev['uid'],
                                ...
                                else
                                {
                                    ...
                                    //User next approval is empty (role not assign)
                                    $workflowName =  $v['workflow_item_name'];
                                    $roles = $masterRoleType->fetchRow("id=$roleID");
                                    $roleName = $roles['display_name'];
                                    $msgArray = array(
                                        "success" => false,
                                        "msg" => "Next person for this Workflow is not Exist, Please contact IT Support.<br><b>Error Reason : " .
                                        "Workflow Generic " . $workflowName . " " . $v['prj_kode'] . " User Role : " . $roleName . " Not Found.</b>"
                                    );
