...
var superTable = function (tableId, options, print) {
/////* Initialize */
    if (print == undefined)
        print = false;
...
/////* Set up the colGroup */
	...
	for (i=0, j=alpha[cleanRow].cells.length; i<j; i++) {
		if (i === this.colWidths.length || this.colWidths[i] === -1) {
			getWidth = alpha[cleanRow].cells[i].getAttribute('width');
			if (getWidth == undefined || getWidth == null)
				this.colWidths[i] = alpha[cleanRow].cells[i].offsetWidth;
			else
				this.colWidths[i] = getWidth;
		}
	}
	for (i=0, j=this.colWidths.length; i<j; i++) {
		this.sColGroup.appendChild(document.createElement("COL"));
		getStyle = alpha[cleanRow].cells[i].getAttribute('style');
		if (getStyle == undefined || getStyle == null)
			this.sColGroup.lastChild.setAttribute("style", getStyle);
