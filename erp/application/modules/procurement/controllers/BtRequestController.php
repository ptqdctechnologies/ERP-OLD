    ...
    public function editAction() {
     ...
     public function revisionAction() {
        ...
        foreach ($payment as $k => $v) {
            ...
            $payment[$k]['others'] = floatval($payment[$k]['others']);
            $payment[$k]['total'] = floatval($payment[$k]['total']);
            // bypasss $payment[$k]['brfp'] = $payment[$k]['sequence']==$cek['sequence'] ? $tranoEdit : '';
        ...
        $this->view->data = $data;
        $this->view->payment = $this->sortPayment($payment);
        // bypass $this->view->paymentbrfp = $totalPayment;
        $this->view->transport = $transport;
        $this->view->type = $type;
        $this->view->paymentbrf = $totalPayment;
...
//                $cost = $v['po_USD'] + $v['arfasf_no_brf_USD'] + ($brdCost[0]['totalHargaUSD'] - $v['arfasf_brf_USD']) + $v['arfasf_brf_USD'];
    ...
    public function isPaid($trano)
    {
            ...
            $sql = "SELECT COUNT(*) As exist FROM erpdb.finance_payment_arf WHERE doc_trano='$trano' ";
    ...
    public function isPartial($trano){
        $sql = "SELECT travel_arrangement,IF(travel_arrangement IN('long','short','day_trip') && totalsequence='1','0','1') As exist  FROM erpdb.procurement_brfh WHERE trano='$trano'";
...
//                        $this->view->allReject = $allReject;
        ...
        if ($data['statrevisi'] > 0 || $this->isUpdate($trano)!=0){
...
//            $trano = $tranoEdit;
    ...
    public function cekWorkflowAction() {
        ...
        $valid = true;
        $msg = null;
        if($item_type == 'BSF' ||$item_type == 'BRFP' ){
...
//            $msg = (string)$select;
    ...
    public function historyBrfAction() {
        ...
        if (count($cek)) 
        {
            ...
            foreach ($cek as $k => $v) {
                ...
                $arrayAfter[$count]['header'] = $after['brf-header-after'];
                $arrayAfter[$count]['detail'] = $after['brf-detail-after'];
                
                $arrayBefore[$count]['header'] = $before['brf-header-before'];
                $arrayBefore[$count]['detail'] = $before['brf-detail-before'];
                
                
                $arrayAfter[$count]['action'] = $v['action'];
