...
<?php if ($this->json != '') {  ?>
    ...
    storeARF.each(function(stores) {
        var id = stores.data['boq_id'];
        if(newRequests[id] == undefined)
        {
            newRequests[id] = parseFloat(stores.data['qty']) * parseFloat(stores.data['harga']);
        }
        else
        {
            newRequests[id] += parseFloat(stores.data['qty']) * parseFloat(stores.data['harga']);
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            store: new Ext.data.SimpleStore({
                                fields:['nilai', 'ori'],
                                data:[['Project','Project'],['Overhead','Overhead'],['Sales','Sales']]
                            }),
                            valueField:'ori',
                            displayField:'nilai',
                            typeAhead: true,
                            ...
                            listeners : {
                               'select' : function (c,stores, index){
                                    ...
                                    else if (data == 'Overhead')
                                    {
                                        myPanel = Ext.getCmp('abs-budget-panel');
                                        myPanel.body.load({
                                            url: '/procurement/procurement/addnewarfbudget',
                                            scripts : true
                                        });
                                    }
                                }
...
var userColumns = [
    ...
    {header: "Work Id", width: 85, sortable: true, dataIndex: 'workid'},
    {header: "Work Name", width: 150, sortable: true, dataIndex: 'workname'},
    {header: "Product Id", width: 90, sortable: true, dataIndex: 'kode_brg'},
    ...
    {header: "Price", width: 80, sortable: true, dataIndex: 'price', css: 'text-align:right;', renderer: function(v, p, r) {
...
var userColumns2 =  [
    ...
    {header: "Delete", width: 50, dataIndex: 'id', css: 'text-align:center;', renderer: function(v, p, r) {
        return '<a href="#" onclick="delToARF(' + v + ',\'' + r.data['workid'] + '\');"><img src="/images/g_rec_del.png"></a>';
    }},
...
var userGrids2 = new userGrid2({
	...
	handler: function()
	{
	    ...
	    Ext.getCmp('arf-grid').getStore().each(function(store){json += Ext.util.JSON.encode(store.data) + ',';});
            ...
            myPanel.body.load({
                url: '/procurement/procurement/apparf', //the url
                scripts : true,
...
function insertToARF()
{
    ...
    if (cfsKode == "" || cfsKode == undefined || cfsKode == '""' || cfsKode == 'x' || cfsKode == 'X' || cfsKode == 'Invalid')
    {
        Ext.MessageBox.show({
            title: 'Error',
            msg: 'Please fill Net Act with CFS Code!',
            buttons: Ext.MessageBox.OK,
            icon: Ext.MessageBox.ERROR
        });
        return false;
    ...
    var arf = storeARF.queryBy(function(record2,id2){
        return record2.get('workid') == Ext.getCmp('arf-workid').getValue()  && record2.get('kode_brg') == Ext.getCmp('kode_brg_text').getValue();
    });
    ...
    if(arf.length !=0)
    {
        if(Ext.getCmp('arf-status').getValue()=='edit')
        {
            ...
            arf.items[0].data['net_act'] = Ext.getCmp('net-act').getValue();
    ...
    else
    {
        var e = new arflist({
            ...
            workid: Ext.getCmp('arf-workid').getValue(),
            workname: Ext.getCmp('arf-workname').getValue(),
...
function editToARF(id)
{
    ...
    Ext.getCmp('arf-workid').setValue(arf.data['workid']);
    Ext.getCmp('arf-workname').setValue(arf.data['workname']);
    ...
    Ext.getCmp('net-act').setValue(arf.data['net_act']);
...
function addToARF(idBoq)
{
    ...
    if (ori != undefined)
    {
        ...
        Ext.getCmp('arf-workname').setValue(ori.data['workname']);
...
function boq3Reload(prjKode,sitKode,valuta)
{
    var newUrl = '/procurement/procurement/getboq3forproject/prj_kode/' + prjKode + '/sit_kode/' + sitKode + '/val_kode/' + valuta;
