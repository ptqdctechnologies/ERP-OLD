...
class Default_Models_Cost extends Zend_Db_Table_Abstract {
    ...
    public function totalOCA($prj_kode='', $sit_kode='', $workid='',$kode_brg='')
    {
        ...
        $sql = "SELECT  
                ...
                FROM erpdb.procurement_asfdd WHERE trano LIKE '%OCA%' AND prj_kode='$prj_kode' $s_sit $s_brg $s_work ;";
    ...
    public function getPO($trano,$workid,$kode_brg)
    {
        $sql = "SELECT  
                ...
                COALESCE(SUM(CASE val_kode WHEN 'USD' THEN qty*harga*rateidr ELSE qty*harga END),0) AS total
                FROM erpdb.procurement_pod WHERE pr_no='$trano' AND workid='$workid' AND kode_brg='$kode_brg' AND approve !=300;";
    ...
    {
        $sql = "SELECT COALESCE(SUM(qty),0) AS qty,COALESCE(SUM(qty*harga),0) AS total FROM erpdb.procurement_pointd 
    ...
    public function totalRequests($prj_kode='', $sit_kode='', $workid='',$kode_brg='',$trano='')
    {
        ...
        $sql1 ="INSERT INTO TotalRequest (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT 
                    a.*
                FROM (SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'PR',trano, val_kode, rateidr,
                        ...
                        WHERE  prj_kode='$prj_kode' $s_sit $s_brg $s_work $s_trano) a
                LEFT JOIN  erpdb.procurement_prh b
		ON (a.trano = b.trano) where approve !=300 or revisi is not null ;";
        ...
        $sql2 ="INSERT INTO TotalRequest (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                 SELECT 
                    a.*
                FROM (SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'ARF',trano, val_kode, rateidr,
                        COALESCE(harga*qty,0),
                        COALESCE(CASE val_kode WHEN 'IDR' THEN qty*harga ElSE qty*harga*rateidr END,0),
                        petugas
                        FROM erpdb.procurement_arfd
                        WHERE prj_kode='$prj_kode' $s_sit $s_brg $s_work AND trano LIKE '%ARF%') a
                   LEFT JOIN erpdb.procurement_arfh b 
                    ON (a.trano = b.trano) where approve !=300 or statrevisi!='0' ;";
        ...
        $sql3 ="INSERT INTO TotalRequest (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT 
                    a.*
                FROM (SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'BRF',trano, val_kode, rateidr,
                        COALESCE(harga*qty,0),
                        COALESCE(CASE val_kode WHEN 'IDR' THEN qty*harga ElSE qty*harga*rateidr END,0),
                        uid
                        FROM erpdb.procurement_brfd
                        WHERE  prj_kode='$prj_kode' $s_sit $s_brg $s_work) a
                   LEFT JOIN erpdb.procurement_brfh b 
                    ON (a.trano = b.trano) where approve !=300 or statrevisi!='0' ;";
    ...
    public function paidCostPerDate($prj_kode='', $sit_kode='', $start_date='', $end_date='')
    {
        ...
        $sql6 ="INSERT INTO PaidCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                ...
                d.uid
                FROM erpdb.finance_payment_rpi d
                INNER JOIN erpdb.procurement_rpih h ON (d.doc_trano = h.trano)
                WHERE d.stspayment ='Y' AND h.statusbrg='SITE' AND d.prj_kode='$prj_kode' $s_sit_rpi $s_date_rpi ;";
    ...
    public function recordedCostPerDate($prj_kode='', $sit_kode='', $start_date='', $end_date='')
    {
        ...
        $sql6 ="INSERT INTO RecordedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                ...
                d.petugas
                FROM erpdb.procurement_rpid d INNER JOIN erpdb.procurement_rpih h ON (d.trano = h.trano)
                WHERE  d.prj_kode='$prj_kode' $s_sit_rpi $s_date_rpi AND d.approve !=300 AND h.statusbrg='SITE' ; ";
    ...
    public function committedCostPerDate($prj_kode='', $sit_kode='', $start_date='', $end_date='')
    {
        ...
        $sql7=" INSERT INTO CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author) 
                ...
                FROM erpdb.procurement_pointd dor 
                LEFT JOIN erpdb.procurement_pod pod ON (pod.pr_no = dor.pr_no AND pod.kode_brg = dor.kode_brg)
    ...
    public function committedCostTotal($prj_kode='', $sit_kode='', $workid='',$kode_brg='')
    {
        ...
        // Buat Temporary Table CommittedCost
        $sql ="DROP TEMPORARY TABLE IF EXISTS erpdb.CommitedCost;
                CREATE TEMPORARY TABLE erpdb.CommitedCost
                (
                    id int(11) NOT NULL AUTO_INCREMENT,
                    prj_kode varchar(255) DEFAULT '',
                    prj_nama varchar(255) DEFAULT '',
        ...
        // INSERT APPROVED ARF
        $sql1 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'ARF',trano, val_kode, rateidr,
                COALESCE(harga*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN qty*harga ElSE qty*harga*rateidr END,0),
                petugas
        ...
        // INSERT APPROVED BRF
        $sql2 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'BRF',trano, val_kode, rateidr,
                COALESCE(harga*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN qty*harga ElSE qty*harga*rateidr END,0),
                uid
        ...
        // INSERT PIECEMEAL
        $sql3 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'Piecemeal',notran, val_kode, rateidr,
                COALESCE(harga_borong*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN qty*harga_borong ElSE qty*harga_borong*rateidr END,0),
                ''
        ...
        // INSERT Salary, Jamsostek, Personal Income Tax (VERSI SEBELUM OCA)
        $sql4 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl, 'OCA', trano, val_kode, rateidr, COALESCE(qty*harga,0), COALESCE(CASE val_kode WHEN 'IDR' THEN (qty*harga) ElSE qty*harga*rateidr END,0), petugas
                FROM erpdb.procurement_sald 
        ...
        // INSERT OCA
        $sql5 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl, 'OCA', trano, val_kode, rateidr, COALESCE(qty*harga,0), COALESCE(CASE val_kode WHEN 'IDR' THEN (qty*harga) ElSE qty*harga*rateidr END,0), petugas
                FROM erpdb.procurement_asfdd
        ...
        // INSERT APPROVED PO
        $sql6 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'PO',trano, val_kode, rateidr,
                CASE COALESCE(qtyspl*hargaspl,0) WHEN 0 THEN COALESCE(qty*harga,0) ELSE COALESCE(qtyspl*hargaspl,0) END,
                COALESCE(CASE val_kode WHEN 'IDR' THEN CASE COALESCE(qtyspl*hargaspl,0) WHEN 0 THEN COALESCE(qty*harga,0) ELSE COALESCE(qtyspl*hargaspl,0) END ElSE CASE COALESCE(qtyspl*hargaspl*rateidr,0) WHEN 0 THEN COALESCE(qty*harga*rateidr,0) ELSE COALESCE(qtyspl*hargaspl*rateidr,0) END END,0),
                petugas
        ...
        // INSERT APPROVED DOR DARI PR YANG TIDAK DIJADIKAN PO
        $sql8=" INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author) 
                SELECT dor.prj_kode,dor.prj_nama,dor.sit_kode,dor.sit_nama,dor.tgl,'DOR',dor.trano,dor.val_kode,dor.rateidr,COALESCE(dor.qty*dor.harga,0),(CASE dor.val_kode WHEN 'IDR' THEN COALESCE(dor.qty*dor.harga,0) ElSE (COALESCE(dor.qty*dor.harga,0)*dor.rateidr) END) , dor.petugas
                FROM erpdb.procurement_pointd dor 
                LEFT JOIN erpdb.procurement_pod pod ON (pod.pr_no = dor.pr_no AND pod.kode_brg = dor.kode_brg)
        ...
        // INSERT MATERIAL CANCEL
        $sql9 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'Material Cancel',trano, val_kode, rateidr,
                COALESCE(-1*harga*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN -1*qty*harga ElSE -1*qty*harga*rateidr END,0),
                petugas
        ...
        // INSERT MATERIAL RETURN
        $sql10 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'Material Return',trano, val_kode, rateidr,
                COALESCE(-1*harga*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN -1*qty*harga ElSE -1*qty*harga*rateidr END,0),
                petugas
        ...
        // INSERT APPROVED ASF DUE TO COMPANY
        $sql11 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'ASF Due to Company',trano, val_kode, rateidr,
                COALESCE(-1*harga*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN -1*qty*harga ElSE -1*qty*harga*rateidr END,0),
                petugas
        ...
        $this->db->query($sql11);
         ...
         // INSERT APPROVED BSF DUE TO COMPANY
        $sql12 ="INSERT INTO erpdb.CommitedCost (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                SELECT prj_kode, prj_nama, sit_kode, sit_nama, tgl,'BSF Due to Company',trano, val_kode, rateidr,
                COALESCE(-1*harga*qty,0),
                COALESCE(CASE val_kode WHEN 'IDR' THEN -1*qty*harga ElSE -1*qty*harga*rateidr END,0),
                petugas
        ...
        $sql13 = 'SELECT prj_kode,
                    ...
                    SUM(amount) AS amount,
                    SUM(total) AS total,
                    author FROM erpdb.CommitedCost;';
    ...
    public function totalRequestsV2($prj_kode='', $sit_kode='', $workid='',$kode_brg='',$trano='')
    {
        ...
        $sql1 ="INSERT INTO totalRequests (prj_kode, prj_nama, sit_kode, sit_nama, tgl,kategori,trano, val_kode, rateidr,amount,total,author)
                ...
                petugas
                FROM erpdb.procurement_arfd
                WHERE  prj_kode='$prj_kode' $s_sit $s_brg $s_work AND trano LIKE 'ARF%' 
                    ) a
                LEFT JOIN erpdb.procurement_arfh b 
                    ON (a.trano = b.trano) where approve !=300 or statrevisi!='0' ; ";
