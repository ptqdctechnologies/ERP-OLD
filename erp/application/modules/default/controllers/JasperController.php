...
//        $print = $printname['displayname'][0];
        ...
        $basePath = $this->reportPath . '/images/Qdc.jpg';
    ...
    public function getdatastockcardAction() {
        ...
        foreach ($data['data'] as $k => $v) {
            ...
            $datas->add(array(
                ...
                "kode_brg" => $v['kode_brg'],
                "nama_brg" => $v['nama_brg'],
                "tanggal" => date("Y-m-d", strtotime($v['tgl'])),
                "gdg_nama" => $v['gdg_nama'],
                "uom" => $v['sat_kode'],
                "masuk" => number_format($v['saldo_masuk'], 4),
                "keluar" => number_format($v['saldo_keluar'], 4),
                "saldo" => number_format($v['saldo_awal'], 4),
                "balance" => number_format($v['balance'], 4)
            ));
        }
        $type = $request->getParam('type');
        if ($type == '')
...
//        $signature = md5(date("Y-m-d H:i:s"). "|" . $uid);
        ...
        $params = array(
            ...
            'cus_nama' => $cus_nama,
            'logoPath' => $this->reportPath . '/images/Qdc.jpg',
...
//        $outputPath = $this->getReportNoDataSource("pdf", $datas, $params, 'bank_receive_money.jrxml', 'Bank Receive Money ' . $trano);
    ...
    public function outprpodetnewAction() {
        ...
        $outputPath = $this->getReport($type, $params, 'outprpodetnew.jrxml', 'outprpodetnew');
    }
}
