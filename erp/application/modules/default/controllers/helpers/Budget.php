...
Zend_Controller_Action_Helper_Abstract {
    ...
    function __construct() {
        ...
        $this->cost = Zend_Controller_Action_HelperBroker::getStaticHelper('Cost');
    ...
    function getCurrentBudget($current_boq3, $recorded_cost, $rateidr) {
        $recorded_cost_usd = 0;
        $recorded_cost_usd_convertion = 0;

        foreach ($recorded_cost AS $index => $data) {
            $recorded_cost_usd += $data['val_kode'] == 'USD' ? $data['amount'] : 0;
            $recorded_cost_usd_convertion += $data['val_kode'] == 'USD' ? $data['total'] : 0;
        }
        //Rumus BOQ3 CURRENT TOTAL = BOQ3 CURRENT IDR + RECORDED COST USD*TRANSACTION'S RATE + (BOQ3 CURRENT USD - RECORDED COST USD)*TODAY'S RATE
        //$boq3_total = $current_boq3['totalIDR'].' + '.$recorded_cost_usd_convertion.' + '.' ( '.$current_boq3['totalUSD'].' - '.$recorded_cost_usd.' ) '.' * '.$rateidr; 
        $current_budget = $current_boq3['totalIDR'] + $recorded_cost_usd_convertion + ( $current_boq3['totalUSD'] - $recorded_cost_usd )  * $rateidr; 
...
//	           }
    ...
    function transferTempOHP($trano) {
        $counter = new Default_Models_MasterCounter();
        $PRAOHPD = new HumanResource_Models_TemporaryOHP();
        $PRAOHPH = new HumanResource_Models_TemporaryOHPH();
        $OHPD = new HumanResource_Models_OHP();
        $OHPH = new HumanResource_Models_OHPH();
        $SALD = new HumanResource_Models_SalaryD();
        $SALH = new HumanResource_Models_SalaryH();

        $praohpd_data = $PRAOHPD->fetchAll("trano = '$trano'");

        if ($praohpd_data) {
            $hasil_praophd = $praohpd_data->toArray();
            $type_ohp = 'OHP';
            $type_sal = 'SAL';
            $trano_ohp = $counter->setNewTrans($type_ohp);
            $trano_sal = $counter->setNewTrans($type_sal);

            foreach ($hasil_praophd as $key => $val) {
                $insert_ohpd = array(
                    "trano" => $trano_ohp,
                ...
                $OHPD->insert($insert_ohpd);

                $insert_sald = $insert_ohpd;
                $insert_sald['trano'] = $trano_sal;
                $SALD->insert($insert_sald);
            }
        }

        $praophh_data = $PRAOHPH->fetchAll("trano = '$trano'");

        if ($praophh_data) {
            $hasil_praophh = $praophh_data->toArray();
            $type_ohp = 'OHP';
            $type_sal = 'SAL';
            $trano_ohp = $counter->setNewTrans($type_ohp);
            $trano_sal = $counter->setNewTrans($type_sal);

            foreach ($praophh_data as $key2 => $val2) {
                $insert_ohph = array(
                    "trano" => $trano_ohp,
                    "ref_number" => $val2['trano'],
                    "tgl" => $val2['tgl'],
                    "genoprj" => $val2['genoprj'],
                    "prj_kode" => $val2['prj_kode'],
                    "prj_nama" => $val2['prj_nama'],
                    "sit_kode" => $val2['sit_kode'],
                    "sit_nama" => $val2['sit_nama'],
                    "ket" => $val2['ket'],
                    "petugas" => $val2['petugas'],
                    "myob" => $val2['myob'],
                    "total" => $val2['total'],
                    "user" => $val2['user'],
                    "tglinput" => $val2['tglinput'],
                    "jam" => $val2['jam'],
                    "val_kode" => $val2['val_kode']
                );

                $OHPH->insert($insert_ohph);

                $insert_salh = $insert_ohph;
                $insert_salh['trano'] = $trano_sal;
                $SALH->insert($insert_salh);
            }
        }
...
//            $type_ohp = 'OHP';
//
//            $trano_ohp = $this->counter->setNewTrans($type_ohp);
