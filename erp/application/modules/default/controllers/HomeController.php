...
//                $manager[$uid] = $acc['displayname'][0];
                $manager[$uid] = QDC_User_Ldap::factory(array("uid" => $uid))->getName();

                $temps = $this->workflowClass->getDocument(array(
                    "uid" => $uid,
                    "itemType" => $searchType,
                    "trano" => $searchID,
                    "prj_kode" => $prjKode,
                    //"count" => $count,
                    "count" => $pa==true ? $count : false,
                    "offset" => $offset,
                    "limit" => $count==true ? 1000 : $limit
                ));
                if ($count) {
                    //$docs += $temps;
                    $docs += $pa==true ? $temps : count($temps);
                } else {
            ...
            $docs = $this->workflowClass->getDocument(array(
                "uid" => $uid,
                "itemType" => $searchType,
                "trano" => $searchID,
                "prj_kode" => $prjKode,
...
//                    $usernamePrev = $otherUserName['displayname'][0];
                    $usernamePrev = QDC_User_Ldap::factory(array("uid" => $val['uid']))->getName();
                } else {
...
$tgl_doc=$val['date'];
$item_id=$val['item_id'];
$ttgl = ((abs(strtotime ($today) - strtotime ($tgl_doc)))/(60*60*24));
if($ttgl > 5){        
    if ($val['approve'] == $this->const['DOCUMENT_REJECT']) {
        if($val['item_type'] == 'ARF' || $val['item_type'] == 'ARFO' ||  $val['item_type'] == 'ARFP' ||$val['item_type'] == 'BRF' || $val['item_type'] == 'PR' || $val['item_type'] == 'PRO' || $val['item_type'] == 'Piecemeal' || $val['item_type'] == 'OCA' ){
            $arrayInsert = array(
                "outstanding" => 1,
            );
            $this->workflowTrans->update($arrayInsert,"item_id = '$item_id'");
        }
    }
        }
...
//                            "signature" => $val['signature'],
            ...
            $docs2 = $pa==true ? $docs : count($docs['posts']);
            $return = array('success' => true, 'count' => $docs2);
        }
        ...
        if ($type) {
            ...
            switch ($type['name']) {
                ...
                case 'ILOV':
                    ...
                    break;
                case 'BRF':
                    $url = "/logistic/logistic/appilov/approve/1/trano_show/$id/show/true";
                    break;
            }
...
//result encoded in JSON
    ...
    public function getdocumenttypeAction() {
        ...
        if ($type['name'] == 'ARF' || $type['name'] == 'ARFO') {
        ...
        else if ($type['name'] == 'PR' || $type['name'] == 'PRO') {
            $where1 = "workflow_trans_id = $id";

            $workflow = $this->workflowTrans->fetchRow($where1);

            $prtrano = $workflow['item_id'];

            $query = "SELECT trano,revisi,tipe FROM procurement_prh WHERE trano = '$prtrano' AND revisi = 1";
            $fetch = $this->db->query($query);
            $data = $fetch->fetch();
            ...
            if ($data) {
                if ($data['tipe'] == 'P') {
                    ...
                    $type['name'] = 'PR_REVISI_OH';
                } else if ($data['tipe'] == 'S') {
                    $type['name'] = 'PR_REVISI_S';
                }
            }
        }
...
//                            $name = $othername['displayname'][0];
                            $name = QDC_User_Ldap::factory(array("uid" => $fetch2['uid']))->getName();
...
//                                    $name = $othername['displayname'][0];
                                    $name = QDC_User_Ldap::factory(array("uid" => $val2['uid']))->getName();
                                    $mail = $othername['mail'][0];
...
//                                    $name = $othername['displayname'][0];
                                    $name = QDC_User_Ldap::factory(array("uid" => $ceks['uid']))->getName();
...
//                                        $name = $othername['displayname'][0];
                                        $name = QDC_User_Ldap::factory(array("uid" => $next['uid_next']))->getName();
...
//                    $name = $othername['displayname'][0];
                    $name = QDC_User_Ldap::factory(array("uid" => $uidStart))->getName();
                    $mail = $othername['mail'][0];
                    if ($fetch['approve'] == $this->const['DOCUMENT_SUBMIT']) {
                        $approve = 'SUBMITTED';
                    } elseif ($fetch['approve'] == $this->const['DOCUMENT_RESUBMIT']) {
        ...
        if ($prjKode != '') {
            ...
            foreach ($boq3 as $key => $val) {
                ...
                if ($current < ($limit + $offset) && $current >= $offset) {
                    ...
                    $boq3[$key]['totalPR'] = floatval($pmeal['qty']) + floatval($pr['qty']) - floatval($ican['qty']);
                    if ($boq3[$key]['val_kode'] == 'IDR') {
                        $boq3[$key]['price'] = $boq3[$key]['hargaIDR'];
//                    $boq3[$key]['totalPricePurchase'] = floatval($po['totalIDR']) + floatval($asfdd['totalIDR']);
                        $boq3[$key]['totalPricePurchase'] = floatval($po['totalIDR']) + (floatval($arf['totalIDR']) - floatval($asfcancel['totalIDR']));
...
//                    $boq3[$key]['totalPricePurchase'] = floatval($po['totalHargaUSD']) + floatval($asfdd['totalHargaUSD']);
                        $boq3[$key]['totalPricePurchase'] = floatval($po['totalHargaUSD']) + (floatval($arf['totalHargaUSD']) - floatval($asfcancel['totalHargaUSD']));
...
//        $boq3['totalPR'] = intval($pr['qty']);
        if ($boq3['val_kode'] == 'IDR') {
            $boq3['price'] = $boq3['hargaIDR'];
//                    $boq3['totalPricePurchase'] = intval($po['totalIDR']) + intval($asfdd['totalIDR']);
            $boq3['totalPricePurchase'] = intval($po['totalIDR']) + (intval($arf['totalIDR']) - intval($asfcancel['totalIDR']));
            $boq3['totalPricePR'] = intval($pr['totalIDR']) + (intval($arf['totalIDR']) - intval($asfcancel['totalIDR']));
        } else {
            $boq3['price'] = $boq3['hargaUSD'];
//                    $boq3['totalPricePurchase'] = intval($po['totalHargaUSD']) + intval($asfdd['totalHargaUSD']);
            $boq3['totalPricePurchase'] = intval($po['totalHargaUSD']) + (intval($arf['totalHargaUSD']) - intval($asfcancel['totalHargaUSD']));
            $boq3['totalPricePR'] = intval($pr['totalHargaUSD']) + (intval($arf['totalHargaUSD']) - intval($asfcancel['totalHargaUSD']));
        }
        $result[] = $boq3;
...
//                var_dump($po);die;
        ...
        if ($pr != '') {
            $boq3['totalqtyPR'] = $pr['qty'];
            if ($boq3['val_kode'] == 'IDR')
                $boq3['totalPR'] = $pr['totalIDR'];
            else
                $boq3['totalPR'] = $pr['totalUSD'];
        }
        else {
            $boq3['totalqtyPR'] = 0;
            $boq3['totalPR'] = 0;
...
//                $totalpoarfasfc = ((intval($boq3['totalPO']) +  intval($boq3['totalARF']) ) -  intval($boq3['totalASFCancel']) ) ;
        $totalpoarfasfc = ((intval($boq3['totalPR']) + intval($boq3['totalARF']) ) - intval($boq3['totalASFCancel']) );
...
//        var_dump($json);die;
    ...
    public function getdorsummaryAction() {
        $this->_helper->viewRenderer->setNoRender();
        $pr_no = $this->getRequest()->getParam('pr_no');

        $sql = 'SELECT SQL_CALC_FOUND_ROWS * FROM procurement_prd WHERE trano = \'' . $pr_no . '\' ORDER BY trano ASC';
        $fetch = $this->db->query($sql);
        ...
        foreach ($return as $key => $value) {
            ...
            $product = $this->quantity->getProductInfo($return[$key]['kode_brg']);
            $return[$key]['uom'] = $product['sat_kode'];
            $return[$key]['hargaavg'] = floatval($product['hargaavg']);
            $return[$key]['nama_brg'] = str_replace("\"", "'", $return[$key]['nama_brg']);

            $dor = $this->quantity->getDorQuantity($return[$key]['prj_kode'], $return[$key]['sit_kode'], $return[$key]['workid'], $return[$key]['kode_brg'], $pr_no);
            if ($dor != '') {
...
//result encoded in JSON
    ...
    public function getdosummaryAction() {
        ...
        foreach ($return as $key => $value) {
            ...
            if ($value['sts_internal'] == 1)
                $return[$key]['qty'] = $return[$key]['qty_int'];
...
//result encoded in JSON
    ...
    public function getprsummaryAction() {
        $this->_helper->viewRenderer->setNoRender();
        $columnValue = $this->getRequest()->getParam('trano');
        $columnValue2 = $this->getRequest()->getParam('kodebrg');
        
        if($columnvalue2 == ''){
            $secondcond = "";
        } else {
             $secondcond = " and kode_brg = '".$columnValue2."' ";
        }

        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM procurement_prd WHERE trano = '" . $columnValue . 
                "' ".$secondcond." ORDER BY trano ASC";
        ...
        foreach ($return as $key => $value) {
            ...
            $return[$key]['id'] =  (int) $return[$key]['id'];
            $return[$key]['uom'] = $this->quantity->getUOMByProductID($return[$key]['kode_brg']);
...
//	    		}
            ...
            /* ---------------------------- Comment Check Budget from BOQ3 untuk Misc workid & !misc workid -------/
              ...
              //                $boq3 = $this->budget->getBoq3ByOne($return[$key]['prj_kode'],$return[$key]['sit_kode'],$return[$key]['workid'],$return[$key]['kode_brg']);
              //                if($boq3)
              //                {
              //                    if ($return[$key]['val_kode'] == 'IDR')
              //                    {
              ...
              //                }
              //
              //                $pod = $this->quantity->getPoQuantity($return[$key]['prj_kode'],$return[$key]['sit_kode'],$return[$key]['workid'],$return[$key]['kode_brg']);
              //                $arf = $this->quantity->getArfQuantity($return[$key]['prj_kode'],$return[$key]['sit_kode'],$return[$key]['workid'],$return[$key]['kode_brg']);
              //                $asfcancel = $this->quantity->getAsfcancelQuantity($return[$key]['prj_kode'],$return[$key]['sit_kode'],$return[$key]['workid'],$return[$key]['kode_brg']);
              //                if ($pod != '' )
              //                {
              //                    $return[$key]['totalqtyPOD'] = $pod['qty'];
              //                    if ($return[$key]['val_kode'] == 'IDR')
...
//result encoded in JSON
    ...
    public function getrpisummaryAction() {
        ...
        $sql = 'SELECT SQL_CALC_FOUND_ROWS * FROM procurement_rpid WHERE deleted=0 AND trano LIKE \'%' . $columnValue . '%\' ORDER BY trano ASC';
        $fetch = $this->db->query($sql);
        $return = $fetch->fetchAll();
...
//    }
    ...
    public function getprfromdorAction() {
      ...
      public function getarfsummaryAction() {
        $this->_helper->viewRenderer->setNoRender();
        $columnValue = $this->getRequest()->getParam('data');
        $columnCheck = $this->getRequest()->getParam('check');
        
        if ($columnCheck == 0){
            $sql = 'SELECT SQL_CALC_FOUND_ROWS * FROM procurement_arfd WHERE trano LIKE \'%' . $columnValue . '%\' ORDER BY trano ASC';
        } else {
            $sql = 'SELECT SQL_CALC_FOUND_ROWS * FROM procurement_brfd WHERE trano LIKE \'%' . $columnValue . '%\' ORDER BY trano ASC';
        }
...
//result encoded in JSON
    ...
    public function getboq3arfsummaryAction() {
        ...
        $no_bt = $this->getRequest()->getParam("no_bt");
        $current = 0;
...
//                var_dump($po);die;
                ...
                if ($pr != '') {
                    $boq3[$key]['totalqtyPR'] = $pr['qty'];
                    if ($boq3[$key]['val_kode'] == 'IDR')
                        $boq3[$key]['totalPR'] = $pr['totalIDR'];
                    else
                        $boq3[$key]['totalPR'] = $pr['totalHargaUSD'];
                }
                else {
                    $boq3[$key]['totalqtyPO'] = 0;
                    $boq3[$key]['totalPO'] = 0;
...
//result encoded in JSON
    ...
    public function getboq3erfsummaryAction() {
        $this->_helper->viewRenderer->setNoRender();
        $prjKode = $this->getRequest()->getParam("prj_kode");
        $sitKode = $this->getRequest()->getParam("sit_kode");
        $paging = $this->getRequest()->getParam("erf");
        $no_bt = $this->getRequest()->getParam("no_bt");
        $current = 0;
...
//                $id++;
            ...
            if ($current < ($limit + $offset) && $current >= $offset) {
                ...
                $po = $this->quantity->getPoQuantity($prjKode, $sitKode, $boq3[$key]['workid'], $boq3[$key]['kode_brg']);
                $pr = $this->quantity->getPrQuantity($prjKode, $sitKode, $boq3[$key]['workid'], $boq3[$key]['kode_brg']);
                $erf = $this->quantity->getErfQuantity($prjKode, $sitKode, $boq3[$key]['workid'], $boq3[$key]['kode_brg']);
                $esfcancel = $this->quantity->getEsfcancelQuantity($prjKode, $sitKode, $boq3[$key]['workid'], $boq3[$key]['kode_brg']);
                $sal = $this->quantity->getSalQuantity($prjKode, $sitKode, $boq3[$key]['workid'], $boq3[$key]['kode_brg']);
//                $reimburs = $this->quantity->getReimbursementQuantity($prjKode,$sitKode,$boq3[$key]['workid'],$boq3[$key]['kode_brg']);
//                var_dump($po);die;
                if ($po != '') {
                ...
                if ($erf != '') {
                    $boq3[$key]['totalqtyERF'] = $erf['qty'];
                    if ($boq3[$key]['val_kode'] == 'IDR')
                        $boq3[$key]['totalInERF'] = $erf['totalIDR'];
                    else
                        $boq3[$key]['totalInERF'] = $erf['totalHargaUSD'];
                }
                else {
                    $boq3[$key]['totalqtyERF'] = 0;
                    $boq3[$key]['totalInERF'] = 0;
                }

                if ($esfcancel != '') {
                    $boq3[$key]['totalqtyESFCancel'] = $esfcancel['qty'];
                    if ($boq3[$key]['val_kode'] == 'IDR')
                        $boq3[$key]['totalESFCancel'] = $esfcancel['totalIDR'];
                    else
                        $boq3[$key]['totalESFCancel'] = $esfcancel['totalHargaUSD'];
                }
                else {
                    $boq3[$key]['totalqtyESFCancel'] = 0;
                    $boq3[$key]['totalESFCancel'] = 0;
                }
                ...
                $totalpoerfesfc = ((($boq3[$key]['totalPO']) + $boq3[$key]['totalInERF'] + $boq3[$key]['totalSal']) - $boq3[$key]['totalESFCancel'] );

                $boq3[$key]['totalPoErfEsfc'] = $totalpoerfesfc;
                $result[] = $boq3[$key];
            }
            $current++;
            $i++;
...
//                $totalpoarfasfc = (($budget[$key]['totalPO'] +  $budget[$key]['totalInARF']  + $budget[$key]['totalPricePR']) -  $budget[$key]['totalASFCancel'] ) ;
                $totalpoarfasfc = ((($budget[$key]['totalPricePR']) + $budget[$key]['totalInARF']) - $budget[$key]['totalASFCancel'] );
...
//result encoded in JSON
    ...
    public function getbudgeterfsummaryAction() {
        $this->_helper->viewRenderer->setNoRender();
        $prjKode = $this->getRequest()->getParam("prj_kode");
        $sitKode = $this->getRequest()->getParam("sit_kode");
        $paging = $this->getRequest()->getParam("erf");
...
//                $budget[$key]['nama_brg'] = str_replace("'"," inch",$budget[$key]['nama_brg']);
            ...
            if ($current < ($limit + $offset) && $current >= $offset) {
                ...
                $po = $this->quantity->getPoQuantity($prjKode, $sitKode, $budget[$key]['budgetid']);
                $erf = $this->quantity->getErfQuantity($prjKode, $sitKode, $budget[$key]['budgetid']);
                $esfcancel = $this->quantity->getesfcancelQuantity($prjKode, $sitKode, $budget[$key]['budgetid']);
//                $reimburs = $this->quantity->getReimbursementQuantity($prjKode,$sitKode,$budget[$key]['budgetid']);
                $pr = $this->quantity->getPrOverheadQuantity($prjKode, $sitKode, $budget[$key]['budgetid']);
                if ($pr != '') {
...
//                var_dump($po);die;
                ...
                if ($erf != '') {
                    $budget[$key]['totalqtyERF'] = $erf['qty'];
                    if ($budget[$key]['val_kode'] == 'IDR')
                        $budget[$key]['totalInERF'] = $erf['totalIDR'];
                    else
                        $budget[$key]['totalInERF'] = $erf['totalHargaUSD'];
                }
                else {
                    $budget[$key]['totalqtyERF'] = 0;
                    $budget[$key]['totalInERF'] = 0;
                }

                if ($esfcancel != '') {
                    $budget[$key]['totalqtyESFCancel'] = $esfcancel['qty'];
                    if ($budget[$key]['val_kode'] == 'IDR')
                        $budget[$key]['totalESFCancel'] = $esfcancel['totalIDR'];
                    else
                        $budget[$key]['totalESFCancel'] = $esfcancel['totalHargaUSD'];
                }
                else {
                    $budget[$key]['totalqtyESFCancel'] = 0;
                    $budget[$key]['totalESFCancel'] = 0;
                }
...
//                $totalpoarfasfc = (($budget[$key]['totalPO'] +  $budget[$key]['totalInARF'] + $budget[$key]['totalReimburs'] + $budget[$key]['totalPricePR']) -  $budget[$key]['totalASFCancel'] ) ;
//                $totalpoarfasfc = (($budget[$key]['totalPO'] +  $budget[$key]['totalInARF']  + $budget[$key]['totalPricePR']) -  $budget[$key]['totalASFCancel'] ) ;
                $totalpoesfesfc = ((($budget[$key]['totalPricePR']) + $budget[$key]['totalInERF']) - $budget[$key]['totalESFCancel'] );
                $budget[$key]['totalPoErfEsfc'] = $totalpoesfesfc;
                $result[] = $budget[$key];
            }
            $current++;
            $i++;
...
//result encoded in JSON
    ...
    public function getdataforeditasfAction() {
        ...
        $sql = "SELECT arf_no FROM procurement_asfd WHERE trano = '$asfno'";
        $fetch = $this->db->query($sql);
        $ref_no = $fetch->fetchAll();
        
        foreach ($ref_no as $key => $value) {
            $ref_no2 = $value;
        }

        if(strpos($ref_no2['arf_no'], 'BRF-') !== FALSE){
            $sql = "SELECT a.* FROM  procurement_brfd a LEFT JOIN procurement_asfd b ON a.trano = b.arf_no WHERE b.trano = '$asfno'";
        }else{
...
//        if ($key == 'trano' && )
    ...
    public function cekjurnalsettleAction() {
        $this->_helper->viewRenderer->setNoRender();
        $ref_number = $this->getRequest()->getParam('trano');

        $cek = $this->js->fetchRow("ref_number = '$ref_number'");

        if ($cek) {
            $hasil = "{success: false,msg: \"This Document has been journal\"}";
        } else {
            $hasil = "{success: true}";
        }
        $this->getResponse()->setHeader('Content-Type', 'text/javascript');
    ...
    public function cekpaymentAction() {
        ...
        if ($type == 'PO') {
            $this->getResponse()->setHeader('Content-Type', 'text/javascript');
            $this->getResponse()->setBody("{success: true, total : '$cek'}");
        } else if ($cek == 'Y') {
            $this->getResponse()->setHeader('Content-Type', 'text/javascript');
...
//result encoded in JSON
    ...
    public function getlistdocumentbytypeAction() {
        ...
        $prj_kode = $this->getRequest()->getParam('prj_kode');
        $sit_kode = $this->getRequest()->getParam('sit_kode');
        $detailType = $this->getRequest()->getParam('detailType');
        $grouped = $this->getRequest()->getParam('grouped');
        ...
        if ($type == 'ARF Pulsa') {
            ...
            if ($trano)
                $select = $select->where("caption_id LIKE '%$trano%'");
            if ($prj_kode)
                $select = $select->where("prj_kode '$trano'");

            $subselect = $this->db->select()
                    ->from(array("a" => $select), array(
                        new Zend_Db_Expr("SQL_CALC_FOUND_ROWS a.*")
                    ))
                    ->group(array("a.caption_id"))
                    ->order(array("date DESC"))
                    ->limit($limit, $offset);
            ...
            foreach ($data as $k => $v) {
                $data[$k]['prj_kode'] = '';
                $data[$k]['sit_kode'] = '';
                $data[$k]['trano'] = $v['caption_id'];
            }

            $return['posts'] = $data;
            $return['count'] = $count;
        } else if ($type == 'ASF Pulsa') {
            $select = $this->db->select()
                    ->from(array($this->workflowTrans->__name()))
                    ->where("caption_id IS NOT NULL")
                    ->where("item_type = 'ASFP'")
                    ->order(array("date desc"));
            ...
            /*if($type=='RPI' || $type='RPIO')
            {
                $select = $this->db->select()
                    ->from(array($class->__name()))
                        ->where("deleted=0");
            }else
            {*/
                $select = $this->db->select()
...
//                        $query = "$prikey LIKE '%$trano%'";
                        $select->where("$prikey LIKE '%$trano%'");
                    if ($prj_kode) {
//                        if ($query)
//                            $query .= " AND prj_kode LIKE '%$prj_kode%'";
//                        else
//                            $query = "prj_kode LIKE '%$prj_kode%'";
                        $select->where("prj_kode LIKE '%$prj_kode%'");
                    }
                    if ($sit_kode) {
//                        if ($query)
...
//                            $query = "sit_kode LIKE '%$sit_kode%'";
                        ...
                        $select->where("sit_kode LIKE '%$sit_kode%'");
                    }
                }
                if ($detailType) {
//                    if ($query)
//                        $query .= " AND tipe = '$detailType'";
//                    else
//                        $query = "tipe = '$detailType'";

                    $select->where("tipe = '$detailType'");
                }

                if ($grouped)
                    $select->group(array("$prikey"));

                if ($type == 'BRFP' || $type == 'BRF' || $type == 'BSF' || $type == 'ASF') {
                    $select = $select->where("trano LIKE '$type%'");
                }
                /*if ($type == 'ASF') {
                    $select = $select->where("trano LIKE 'ASF01%'");
                }*/
                if ($type == 'ASFO') {
                    $select = $select->where("trano LIKE 'ASF02%'");
                }
                if ($type == 'ARF') {
                    $select = $select->where("trano LIKE '$type%'");
                }
                if ($type == 'ARFO') {
                    $select = $select->where("trano LIKE 'ARF02%'");
                }
                ...
                $select->order(array("id DESC", "$sort $dir"))
                        ->limit($limit, $offset);

                $data = $this->db->fetchAll($select);

//                $data = $class->fetchAll($query, array("id DESC", $sort . ' ' . $dir), $limit, $offset)->toArray();
                ...
                if ($type == 'BRFP' || $type == 'BRF' || $type == 'BSF' || $type == 'CRPI' || $type = 'UHB') {
                    foreach ($data as $key => $val) {
                        $data[$key]['grouped'] = true;
                    }
                }
...
//                        $nextName = $othername['displayname'][0];
                        $nextName = QDC_User_Ldap::factory(array("uid" => $next))->getName();
        ...
        if ($data[(count($data) - 1)]['approve']!=null && $data[(count($data) - 1)]['approve'] < 300) {
            
            $next = $data[(count($data) - 1)]['uid_next'];
            $nextName = QDC_User_Ldap::factory(array("uid" => $next))->getName();
            $this->view->msg = "Awaiting approval from $nextName.";
...
//            $name = $othername['displayname'][0];
            ...
            switch ($val['approve']) {
                ...
                case $this->const['DOCUMENT_FINAL']:
                    $val['approve'] = '<b><font color="blue">Final Approval</font></b>';
                    $finalDate = date("Y-m-d H:i:s", strtotime($val['date']));
                    break;
...
//        $this->view->assetGlpi = $assetGlpi;
        ...
        $workflowGeneric = new Admin_Models_Workflowgeneric();
        $masterRole = new Admin_Models_Masterrole();
        $timesheet = new ProjectManagement_Models_Timesheet();

        $progress = new ProjectManagement_Models_ProjectProgress();
        $projectNotProgress = array();
...
//Buat Tabs...
                ...
                if ($role == 'Project Manager') {
                    $prjArray[] = "'" . $v['prj_kode'] . "'";
                    $prjKode = $v['prj_kode'];
//Get summary last progress...
                    $sumProgress = $progress->getSumProjectProgress($prjKode, false);
                    if ($sumProgress == 0) {
                        $projectNotProgress[] = array(
                            "prj_kode" => $prjKode,
                            "msg" => "Your project : <b>" . $prjKode . "</b> does not have any Progress yet.<br>Please input Project Progress from Project Management -> Project Progress."
                        );
                    } else {
                        $last = $progress->getLastDate($prjKode);
                        if ($last['total_days'] >= 0 && $last['total_days'] >= 14) {
                            $projectNotProgress[] = array(
                                "prj_kode" => $prjKode,
                                "msg" => "Your last progress for project : <b>" . $prjKode . "</b> is " . number_format($last['total_days']) . " days old<br>Please input New Project Progress from Project Management -> Project Progress."
                            );
                        }
                    }
                }
...
//            }
            ...
            $this->view->projectProgressMsg = $projectNotProgress[0]['msg'];
        }
    }
...
//            $name = $other['displayname'][0];
            $name = QDC_User_Ldap::factory(array("uid" => $val['appUid']))->getName();
...
//result encoded in JSON
    ...
    public function showdocssubmitAction() {
        ...
        $sql = "
            SELECT SQL_CALC_FOUND_ROWS a.*,b.approve AS lastApprove,b.date AS lastDate,b.uid AS uidApprove,b.uid_next AS uidNextApprove  FROM my_submit a
        ...
        if ($fetch) {
            ...
            foreach ($hasil as $key => $val) {
                ...
                if ($lastApprove == $this->const['DOCUMENT_APPROVE']) {
                    $nextApproveUID = $val['uidNextApprove'];
                    $nextApproveName = QDC_User_Ldap::factory(array("uid" => $nextApproveUID))->getName();
                    $hasil[$key]['msg'] = "Awaiting approval from $nextApproveName.";
                    ...
                    $nextName = QDC_User_Ldap::factory(array("uid" => $val['uid_next']))->getName();
...
//                $manager['posts'][] = array("name" => $acc['displayname'][0], "manager_uid" => $v['uid_manager']);
                $manager['posts'][] = array("name" => QDC_User_Ldap::factory(array("uid" => $v['uid_manager']))->getName(), "manager_uid" => $v['uid_manager']);
            }
    ...
    public function cekRejectAction() {
        $this->_helper->viewRenderer->setNoRender();
        $trano = $this->getRequest()->getParam('trano');
        $cek = $this->workflow->isDocumentRejected($trano);
        if ($cek !== true) {
            $hasil = "{success: false,msg: \"This Document is not rejected yet.\"}";
        } else {
            $hasil = "{success: true}";
        }
        $this->getResponse()->setHeader('Content-Type', 'text/javascript');
    ...
    public function getDocumentUrlForPrintAction() {
        $this->_helper->viewRenderer->setNoRender();
        $item_id = $this->getRequest()->getParam("trano");
        $type = $this->workflowTrans->getDocumentTypeByTrano($item_id);
        $uid = $this->session->userName;
        ...
        if ($type) {
            switch ($type['name']) {
                case 'PO':
                case 'POO':
                    $url = "/jasper/podetail/trano/$item_id/type/pdf";
                    break;
            }
            ...
            $type = $counter->getTransTypeFlip($item_id);
            switch ($type) {
                case 'BPV':
                    $url = "/jasper/paymentvoucherreport/trano/$item_id/type_doc/pdf";
                    break;
            }
...
//result encoded in JSON
    ...
    public function showCheckDocumentGenericAction() {
        $trano = $this->getRequest()->getParam('trano');
        $grouped = $this->_getParam("grouped");
...
//                $name = $othername['displayname'][0];
                $name = QDC_User_Ldap::factory(array("uid" => $val['uid']))->getName();
                switch ($val['approve']) {
                    case $this->const['DOCUMENT_APPROVE']: $data[$key]['approve'] = '<b><font color="green">Approved</font></b>';
                        break;
                    case $this->const['DOCUMENT_SUBMIT']: $data[$key]['approve'] = '<b>Submitted</b>';
                        break;
                    case $this->const['DOCUMENT_RESUBMIT']: $data[$key]['approve'] = '<b>Resubmitted</b>';
                        break;
                    case $this->const['DOCUMENT_REJECT']: $data[$key]['approve'] = '<b><font color="red">Rejected</font></b>';
                        break;
                    case $this->const['DOCUMENT_FINAL']: $data[$key]['approve'] = '<b><font color="blue">Final Approval</font></b>';
                        break;
                    case $this->const['DOCUMENT_EXECUTE']: $data[$key]['approve'] = '<b><font color="blue">Executed</font></b>';
                        break;
                    default: $data[$key]['approve'] = '';
                        break;
                }
                $data[$key]['date'] = date("D, d/m/Y H:i:s", strtotime($val['date']));
                $data[$key]['name'] = $name;
                if ($data[$key]['comment'] == '')
                    $data[$key]['comment'] = "&nbsp;";
                $data[$key]['comment'] = str_replace("\r", "<br />", $val['comment']);
            ...
            $this->view->data = $data;
            echo $this->view->render('home/showcheckdocument.phtml');
        }
        else {

            $select = $this->db->select()
                    ->from(array($this->workflowTrans->__name()))
                    ->where("caption_id = '$trano'")
                    ->order(array("item_id ASC", "date ASC"));

            $data = $this->db->fetchAll($select);
            $ldap = new Default_Models_Ldap();

//        $this->view->captionID = $trano;
            $this->view->lastTransId = $data[(count($data) - 1)]['workflow_trans_id'];
            $this->view->type = $data[(count($data) - 1)]['item_type'];
            $lastUid = $data[(count($data) - 1)]['uid'];
            $lastWorkflow = $data[(count($data) - 1)]['workflow_id'];
            ...
            foreach ($data as $key => &$val) {
                ...
                $data[$key]['date'] = date("D, d/m/Y H:i:s", strtotime($val['date']));
                $data[$key]['name'] = QDC_User_Ldap::factory(array("uid" => $val['uid']))->getName();
                if ($data[$key]['comment'] == '')
                    $data[$key]['comment'] = "&nbsp;";
                $data[$key]['comment'] = str_replace("\r", "<br />", $val['comment']);
                $data[$key]['comment'] = str_replace("\n", "<br />", $val['comment']);
                ...
                $itemID = $val['item_id'];
                if ($itemType == 'ARFP') {
                    $arfh = $this->PROC->Procurementarfh->fetchAll("trano_ref = '$itemID'");
                    if ($arfh) {
                        $arfh = $arfh->toArray();
                        $refNumber = array();
                        foreach ($arfh as $k => $v) {
                        ...
                        $data[$key]['ref_number'] = implode("<br>", $refNumber);
                    }
                    $this->view->refNumberCaption = 'ARF No';
                }
                
                if ($itemType == 'ASFP') {
                    $arfh = $this->PROC->Procurementasfh->fetchAll("trano_ref = '$itemID'");
                    if ($arfh) {
                        $arfh = $arfh->toArray();
                        $refNumber = array();
                        foreach ($arfh as $k => $v) {
                    ...
                    $this->view->refNumberCaption = 'ASF No';
                }

                $groupID[$itemID]['detail'][] = $data[$key];
                $groupID[$itemID]['waiting'] = '';
                if ($approval != $this->const['DOCUMENT_REJECT'] && $approval != $this->const['DOCUMENT_FINAL']) {
                    $statMsg = "Awaiting Approval from ";
                    $groupID[$itemID]['waiting'] = $statMsg . QDC_User_Ldap::factory(array("uid" => $val['uid_next']))->getName();
                }
            }

            $this->view->data = $groupID;
        }
    }

    public function getBoq3OverheadAction() {
        $this->_helper->viewRenderer->setNoRender();
        $prjKode = $this->getRequest()->getParam("prj_kode");
        $sitKode = $this->getRequest()->getParam("sit_kode");
...
//result encoded in JSON
    ...
    public function getDocumentInMyWorkflowAction() {
        $this->_helper->viewRenderer->setNoRender();
        $prjKode = $this->getRequest()->getParam("prj_kode");
        $itemType = $this->getRequest()->getParam("workflow_item_type_id");
        ...
        $select = $this->db->select()
                ->from(array($this->ADMIN->Workflow->__name()), array(
                    "prj_kode",
                    "workflow_item_id"
                ))
                ->where("uid = ?", $uid)
                ->where("prj_kode = ?", $prjKode)
                ->group(array("prj_kode", "workflow_item_id"));

        if ($itemType) {
            $select = $this->db->select()
                    ->from(array("x" => $select))
                    ->joinLeft(array("y" => $this->ADMIN->Workflowitem->__name()), "x.workflow_item_id = y.workflow_item_id", array())
                    ->where("workflow_item_type_id = ?", $itemType);
        }
        ...
        $select2 = $this->db->select()
                ...
                ->joinLeft(array("b" => $this->ADMIN->Workflowtrans->__name()), "a.workflow_item_id = b.workflow_item_id AND a.prj_kode = b.prj_kode", array(
                    "trano" => "item_id",
                    "last_date" => "date",
                    "uid_next",
                    "uid_prev",
        ...
        $select4 = $this->db->select()
                ->from(array("d" => $select3), array(
                    new Zend_Db_Expr("SQL_CALC_FOUND_ROWS d.*")
                ))
                ->joinLeft(array("e" => $this->ADMIN->Workflowtrans->__name()), "d.trano = e.item_id", array(
                    "submit_date" => "MIN(date)"
                ))
                ->limit($limit, $offset)
                ->order(array("d.last_date DESC"))
                ->group(array("trano"));

        $w = $this->db->fetchAll($select4);
        ...
        foreach ($w as $k => $v) {
            ...
            if ($v['uid_next'] != 'SYSTEM') {
                $user = QDC_User_Ldap::factory(array("uid" => $v['uid_next']))->getName();
            } else {
    ...
    public function getMyProjectAction() {
        $this->_helper->viewRenderer->setNoRender();

        $uid = QDC_User_Session::factory()->getCurrentUID();

        $select = $this->db->select()
                ->from(array("a" => $this->ADMIN->Masterlogin->__name()), array(
                    "id"
                ))
                ->joinLeft(array("b" => $this->ADMIN->Masterrole->__name()), "a.id = b.id_user", array(
                    "prj_kode"
                ))
                ->where("a.uid = ?", $uid)
                ->order(array("prj_kode DESC"))
                ->group(array("prj_kode"));

        $subselect = $this->db->select()
                ->from(array("c" => $select))
                ->joinLeft(array("d" => $this->DEFAULT->MasterProject->__name()), "c.prj_kode = d.Prj_Kode", array(
            "prj_nama" => "Prj_Nama"
        ));

        $prjs = $this->db->fetchAll($subselect);
    ...
    public function documentInMyWorkflowAction() {
        
    }

    public function logPrintDocumentAction() {
        
    }

    public function showLogPrintDocumentAction() {
        $trano = $this->_getParam("trano");

        $l = new Default_Models_LogPrint();
        $cek = $l->fetchAll("trano LIKE '%$trano%'", array("date DESC"));
        $data = array();
        if ($cek) {
            $data = $cek->toArray();
            $person = array();
    ...
    public function getRemarkAllTransactionAction() {
        $this->_helper->viewRenderer->setNoRender();
        $trano = $this->getRequest()->getParam("trano");
        $type = $this->getRequest()->getParam("type");
    ...
    public function updateRemarkAllTransactionAction() {
        $this->_helper->viewRenderer->setNoRender();
        $trano = $this->getRequest()->getParam("trano");
        $type = $this->getRequest()->getParam("type");
        $ket = $this->getRequest()->getParam("ket");
        $paymentterm = $this->getRequest()->getParam("paymentterm");
        ...
        if ($class !== false) {
            ...
            if ($trano != '') {
                ...
                $log['before'] = $this->db->fetchAll($select);*/
                ...
                $update_value = array(
                    "ket" => $ket,
                    "revisi" => intval($log['po-header-before'][0]['revisi']) + 1
                );
                
                //paymentterm
                if ($paymentterm)
                    $update_value['paymentterm'] = $paymentterm;

                $this->db->update($class->__name(), $update_value, " $prikey = '$trano'");
                $return['success'] = true;

                //log after
                //$log2['after'] = $this->db->fetchAll($select);
                ...
                $arrayLog = array(
                    ...
                    "prj_kode" => $log['po-header-before'][0]['prj_kode'],
                    "sit_kode" => $log['po-header-before'][0]['sit_kode'],
                    "action" => "UPDATE REMARKS PO",
                    "data_before" => $jsonLog,
                    "data_after" => $jsonLog2,
    ...
    public function cekAuthRemarksChangerAction() {
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout->disableLayout();

        $username = $this->getRequest()->getParam("username");
        $password = $this->getRequest()->getParam("password");
        $itemType = $this->getRequest()->getParam("itemType");
        ...
        if ($authResult->isValid()) {
            ...
            if ($myRoles) {
                foreach ($myRoles as $k => $v) {
                    ...
                    switch ($itemType) {
                        case 'PO' || 'POO':
                            if ($v['role_name'] == 'Commercial and Procurement' || $v['role_name'] == 'IT System')
                                $found = true;
                            break;
                    }
                }
                ...
                if ($result['auth'] == false) {
                    $result['msg'] = 'Sorry, You dont have credential to Edit this remarks';
                }
            } else {
    ...
    public function getDataProjectAction() {
        $this->_helper->viewRenderer->setNoRender();
        $trano = $this->getRequest()->getParam('data');

        $pr_model = new Default_Models_ProcurementRequest();
        $hasil = $pr_model->getDataProject($trano);
...
//result encoded in JSON
    ...
    public function getDataAdvanceAction() {
        $this->_helper->viewRenderer->setNoRender();
        $trano = $this->getRequest()->getParam('data');

        $arf_model = new Default_Models_AdvanceRequestFormD();
        $hasil = $arf_model->getDataAdvance($trano);
...
//result encoded in JSON
    ...
    public function getlistdocumentbytypenewAction() {
        $this->_helper->viewRenderer->setNoRender();
        $type = $this->getRequest()->getParam('type');
        ...
        if ($type == 'ARF Pulsa') {
            ...
            $model = $this->_helper->getHelper('model');
            $class = $model->getModelNewClass($type);
    ...
    public function cekWorkflowFaAction() {
        $this->_helper->viewRenderer->setNoRender();
        $trano = $this->_getParam("trano");
        ...
        $valid = true;
            if ($isFinal) {
                $msg = "This Document is Final Approval.";
                $valid = false;
            }
