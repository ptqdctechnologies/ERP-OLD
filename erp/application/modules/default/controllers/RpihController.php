...
//    }
    ...
    public function listAction()
    {
        ...
        if ($columnName != '')
            $where = "WHERE tipe = '$columnName' AND deleted=0";
        $offset = (isset($_POST['start'])) ? $_POST['start'] : 0;
        $limit = (isset($_POST['limit'])) ? $_POST['limit'] : 100;
        $sort = (isset($_POST['sort'])) ? $_POST['sort'] : 'trano';
        $dir = (isset($_POST['dir'])) ? $_POST['dir'] : 'ASC';
    ...
    public function listbyparamsAction()
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS trano,DATE_FORMAT(tgl,'%d/%m/%Y') as tgl,prj_kode,prj_nama,sit_kode,sit_nama,po_no,sup_kode,sup_nama,total,totalpo, invoice_no FROM procurement_rpih WHERE $columnName LIKE '%$columnValue%' AND tipe = '$tipe' AND deleted=0 ORDER BY $sort $dir LIMIT $offset,$limit";
	...
	public function listrpidAction()
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM procurement_rpid WHERE deleted=0 AND po_no = '$po_no' ORDER BY " . $sort . ' '  . $dir . ' LIMIT ' . $offset . ',' . $limit;
    ...
    public function getpaymentlistAction()
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS trano,DATE_FORMAT(tgl,\"%m/%d/%Y\") as tgl,prj_kode,prj_nama,sit_kode,sit_nama,po_no,sup_kode,sup_nama,total,totalpo FROM procurement_rpih $where AND deleted=0 ORDER BY " . $sort . ' '  . $dir . ' LIMIT ' . $offset . ',' . $limit;
        $fetch = $this->db->query($sql);
        $return['posts'] = $fetch->fetchAll();
        $return['count'] = $this->db->fetchOne('SELECT FOUND_ROWS()');
        //the posts
