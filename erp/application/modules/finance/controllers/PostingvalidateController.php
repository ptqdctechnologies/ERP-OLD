...
//        var_dump($type,$trano);die;
        ...
        $sql = "
            ...
            SELECT fv.*, ap.rateidr FROM (SELECT trano,valuta,tgl,ref_number,item_type,SUM(total_value) AS total_value, SUM(total_bayar) AS tot_total_bayar, SUM(total) AS total FROM finance_payment_voucher WHERE deleted=0 group by trano,ref_number) fv LEFT JOIN accounting_jurnal_bank ap (ON fv.trano = ap.trano AND fv.ref_number = ap.ref_number AND ap.deleted=0)
            WHERE $where item_type IN ('ARF','REM') AND ap.stspost = 0 group by fv.trano,fv.ref_number ORDER BY $sort $dir;

            INSERT INTO closing
            SELECT fv.*, ap.rateidr FROM (SELECT trano,valuta,tgl,ref_number,item_type,SUM(total_value) AS total_value, SUM(total_bayar) AS tot_total_bayar, SUM(total) AS total FROM finance_payment_voucher WHERE deleted=0 group by trano,ref_number) fv LEFT JOIN accounting_close_ap ap ON (fv.trano = ap.trano AND fv.ref_number = ap.ref_number AND ap.deleted=0)
            WHERE $where fv.item_type = 'RPI' AND ap.stspost = 0 group by fv.trano,fv.ref_number ORDER BY $sort $dir;
        ";
        ...
        if ($fetch)
        {
            ...
            foreach($fetch as $k => $v)
            {
                ...
                $tots = 0;
                            ...
                            {
                                case 'RPI':
                                    $cekPPN = $this->FINANCE->BankPaymentVoucher->fetchAll("trano = '$trano'");
                                    if ($cekPPN)
                                    {
                            $cekPPN = $cekPPN->toArray();
                            foreach($cekPPN as $key => $val)
                            {
                                if ($val['trano_ppn'] != '')
                                {
                                    $select = $this->db->select()
                                        ->from(array($this->FINANCE->BankPaymentVoucher->name()),array(
                                            "total_bayar_ppn" => "SUM(total_bayar)"
                                        ))
                                        ->where("trano = '{$val['trano_ppn']}'");
                                    $bpvPPN = $this->db->fetchRow($select);
                                    if ($bpvPPN)
                                    {
                                        $totsPPN += $bpvPPN['total_bayar_ppn'];
                                    }
                                }
                            }
                        }
                        ...
                        if ($payment)
                        {
                            foreach ($payment as $k2 => $v2)
                            {
                                ...
                                if ($v2['val_kode'] != 'IDR')
                                {
                                    $totsRate += $v2['total_bayar'] * $v2['rateidr'];
                                }
                            }
                        }
                        else
                ...
                $fetch[$k]['total_payment_rate'] = 0;
                $fetch[$k]['total_value_rate'] = 0;
                $fetch[$k]['total_bayar_rate'] = 0;
                $fetch[$k]['total_rate'] = 0;
                if ($type == 'RPI')
                {
                    $fetch[$k]['checkjurnal'] = 0;
                    $fetch[$k]['checkjurnalbank'] = 0;
                    $fetch[$k]['total_value_with_ppn'] = $fetch[$k]['total_value'] + $totsPPN;;
                    $fetch[$k]['total_value'] = $fetch[$k]['total_value'] + $totsPPN;
                    $fetch[$k]['total'] = $fetch[$k]['total'] + $totsPPN;
                }
                else
                {
                ...
                if ($v['valuta'] != 'IDR')
                {
                    $fetch[$k]['total_payment_rate'] = $totsRate;
                    $fetch[$k]['total_rate'] = $fetch[$k]['total'] * $fetch[$k]['rateidr'];
                    $fetch[$k]['total_bayar_rate'] = $fetch[$k]['total_bayar'] * $fetch[$k]['rateidr'];
                    $fetch[$k]['total_value_rate'] = $fetch[$k]['total_value'] * $fetch[$k]['rateidr'];
                }
            }
        }
