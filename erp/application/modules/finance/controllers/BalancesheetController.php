...
class Finance_BalancesheetController extends Zend_Controller_Action
{

    private $layoutbalancesheet;
    private $session;

    public function init()
    {
        $this->layoutbalancesheet = new Finance_Models_LayoutBalanceSheet();
    ...
    public function insertlayoutbalancesheetAction ()
    {
        
    }

    public function doinsertlayoutbalancesheetAction ()
    {
        ...
        $ap = $this->getRequest()->getParam('ap');
        ...
        $cek = $this->layoutbalancesheet->fetchRow($where);
        ...
        if ($cek)
        {
            ...
            $this->layoutbalancesheet->insert($insert);
    ...
    public function doupdatelayoutbalancesheetAction ()
    {
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout->disableLayout();
        ...
        $cek = $this->layoutbalancesheet->fetchRow("id = $idEdit");
        if (!$cek)
        {
        ...
        $arr = explode(".",$oldUrut);
        $oldPrefix = $arr[0];
        $this->layoutbalancesheet->delete("id = $idEdit");

        if ($parent != '')
        {
            $cekParent = $this->layoutbalancesheet->fetchRow("coa_kode = '$parent'");
            if ($cekParent)
            {
        ...
        {
            $cek = $this->layoutbalancesheet->fetchAll("id != $idEdit AND urut LIKE '$oldUrut%'");
            if ($cek)
            {
                $this->layoutbalancesheet->delete("id != $idEdit AND urut LIKE '$oldUrut%'");
            }
            $urut = $this->getUrut($level,$parent);
        }
        else
    ...
    private function getUrut($level=0,$parent='')
    {
        ...
        if ($level == 1)
        {
            $cek = $this->layoutbalancesheet->fetchRow("level = 1",array("urut DESC"));
            if ($cek)
            {
                $cek = $cek->toArray();
        ...
        {
            $cek = $this->layoutbalancesheet->fetchRow("coa_kode = '$parent'");
            if ($cek)
            {
                $prefix = $cek['urut'];

                $cek2 = $this->layoutbalancesheet->fetchRow("level = $level AND urut LIKE '$prefix%'",array("LPAD(urut,30,'0') DESC"));
                if ($cek2)
                {
                    $cek2 = $cek2->toArray();
    ...
    public function editlayoutbalancesheetAction ()
    {
    ...
    public function getcoaAction ()
    {
        ...
        $data['data'] = $this->layoutbalancesheet->fetchAll($where,array($sort . " " . $dir),$limit,$offset)->toArray();

        $data['total'] = $this->layoutbalancesheet->fetchAll($where)->count();
    ...
    public function getcoatreeAction()
    {
        ...
        if ($node == 'posts')
        {
            $firstLevel = $this->layoutbalancesheet->fetchAll("level = 1");
            if ($firstLevel)
            {
        ...
        else
        {
            ...
            $level = count($arr) + 1;
            $i = 0;
            $nextLevel = $this->layoutbalancesheet->fetchAll("level = $level AND urut LIKE '$node.%'");
    ...
    public function getlayoutbalancesheetAction()
    {
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout->disableLayout();
        $id = $this->getRequest()->getParam("id");

        $cek = $this->layoutbalancesheet->fetchRow("id = $id");
        if ($cek)
        {
            $data = $cek->toArray();
            if ($data['level'] > 1 && ($data['hd'] == 'Header' || $data['hd'] == 'Detail'))
            {
                ...
                unset($arr[$count-1]);
                $urut = implode(".",$arr);
                $cek2 = $this->layoutbalancesheet->fetchRow("level = $level AND urut = '$urut'");
                if ($cek2)
                {
    ...
    public function deletelayoutbalancesheetAction ()
    {
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout->disableLayout();
        $id = $this->getRequest()->getParam("id");
        ...
        if ($cek)
        {
            $this->layoutbalancesheet->delete("id = $id");
            $return = array("success" => true);
        }
        else
            $return = array("success" => false, "pesan" => "Invalid Record!");
