...
//            $hasil = $this->remh->fetchAll($search,array($sort . " " . $dir),$limit,$offset)->toArray();
        ...
        }else if ($type == 'PBOQ3')
        {
            ...
            $data['total'] = $this->pmealh->fetchAll()->count();
        }
    ...
    public function doinsertpaymentvoucherAction ()
    {
        ...
        foreach ($voucherdata as $key => $val)
        {
            ...
            elseif ($item_type == 'PBOQ3')
            {
                  $pmeal = $this->pmeald->fetchAll("notran = '$ref_number'")->toArray();

                if ($pmeal)
                {
                    foreach($pmeal as $keypmeal => $valpmeal)
                    {
                        $total = $valpmeal['qty']*$valpmeal['harga_borong'];
                        $insertpmeal = array(
                            'trano' => $trano,
                            'tgl' => $tgl,
                            'uid' => $uid,
                            'ref_number' => $valpmeal['notran'],
                            'prj_kode' => $valpmeal['prj_kode'],
                            'prj_nama' => $valpmeal['prj_nama'],
                            'sit_kode' => $valpmeal['sit_kode'],
                            'sit_nama' => $valpmeal['sit_nama'],
                            'workid' => $valpmeal['workid'],
                            'workname' => $valpmeal['workname'],
                            'kode_brg' => $valpmeal['kode_brg'],
                            'nama_brg' => $valpmeal['nama_brg'],
                            'qty' => $valpmeal['qty'],
                            'harga' => $valpmeal['harga_borong'],
                            'total' => $total,
                            'val_kode' => $valpmeal['val_kode'],
                            'rateidr' => $valpmeal['rateidr'],
                            'cfs_kode' => $valpmeal['cfs_kode'],
                            'coa_kode' => $coa_kode
                        );

                        $this->voucherd->insert($insertpmeal);
                    }
                }
            }
