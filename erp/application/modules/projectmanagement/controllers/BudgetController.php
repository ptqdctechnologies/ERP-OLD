...
class ProjectManagement_BudgetController extends Zend_Controller_Action {
    ...
    private $boq3;
    private $boq3H;
    private $overheadboq3;
    private $overheadboq3H;
    private $cboq3;
    private $cboq3H;
    private $tempBOQ3;
    private $tempBOQ3h;
    private $tempOverheadBOQ3;
    private $tempOverheadBOQ3H;
    private $trans;
...
//		require_once $this->phpexcel.'PHPExcel/IOFactory.php';
        ...
        $this->tempOverheadBOQ3 = new ProjectManagement_Models_TemporaryOverHeadBOQ3d();
        $this->tempOverheadBOQ3H = new ProjectManagement_Models_TemporaryOverHeadBOQ3h();
        $this->overheadboq3 = new ProjectManagement_Models_NonProjectBOQ3();
        $this->overheadboq3H = new ProjectManagement_Models_NonProjectBOQ3h();
...
//                        "prj_kode" => $prjKode,
    ...
    public function uploadboq3Action() {
        ...
        if ($result) {
            ...
            $workid = array();
            $masterWork = new Default_Models_MasterWork();
            foreach ($boq3 as $k => $v) {
                ...
                if ($harga==0) {
                    $boq3[$k]['invalid'] = true;
                    $boq3[$k]['invalid_msg'] = 'Unit Price cannot be empty or 0';
                }
                    
                if ($qty==0) {
                    $boq3[$k]['invalid'] = true;
                    $boq3[$k]['invalid_msg'] = 'QTY cannot be empty or 0';
                }

                //cek kode barang
                if (!QDC_Transaction_Product::factory(array("workid" => $v['workid']))->isMsc()) {
                    $kode_brg = $v['kode_brg'];
                    if (!$this->barang->isExist($kode_brg)) {
                        $boq3[$k]['invalid'] = true;
                        $boq3[$k]['invalid_msg'] = 'Product ID\'s not found on Database';
                    }

                    if ($this->project->isNewProject($prjKode)) {
                        if ($this->barang->isOldMaterial($kode_brg)) {
                            $boq3[$k]['invalid'] = true;
                            $boq3[$k]['invalid_msg'] = 'This Project is NEW, but the Product ID\'s not using NEW material code, please refer to Material List.';
                        }
                    } else {
                        if (!$this->barang->isOldMaterial($kode_brg)) {
                            $boq3[$k]['invalid'] = true;
                            $boq3[$k]['invalid_msg'] = 'This Project is OLD, but Product ID\'s using NEW material code, please refer to Material List.';
                        }
                    }

                    $cekworkid = $v['workid'];
                    if (!$masterWork->isExist($cekworkid)) {
                        $boq3[$k]['invalid'] = true;
                        $boq3[$k]['invalid_msg'] = 'Work ID\'s not found on Database';
                    }

                }
                else
                {
                    $boq3[$k]['kode_brg']= ($v['kode_brg']=='' || $v['kode_brg']==null)  ?'XX' : $v['kode_brg'];
...
//			echo "{success:true}";
    ...
    public function checkbudgetidAction() {
        $this->_helper->viewRenderer->setNoRender();
        Zend_Loader::loadClass('Zend_Json');
        ...
        foreach ($boq3 as $k => $v) {
            $budgetid = $v['budgetid'];
            $cekprabudget = $this->tempOverheadBOQ3->fetchRow("budgetid = '$budgetid'");
            $cekbudget = $this->overheadboq3->fetchRow("budgetid = '$budgetid'");
            if ($cekprabudget || $cekbudget) {
                if ($budgetname != '')
                    $budgetname = $budgetname . ', ' . $v['budgetid'];
                else
                    $budgetname = $v['budgetid'];
            }
        }

        if ($budgetname != '') {
//            $msg = 'This Budget ID = \"$budgetname"'. ' already exists';
            echo "{success:false, msg:'This Budget ID = $budgetname already exists.'}";
        } else
            echo "{success:true}";
    }
...
//		$ket =  Zend_Json::decode($ket);
        ...
        $counter = new Default_Models_MasterCounter();
        $checkco = $BOQ2->fetchRow("prj_kode ='$prjKode' AND sit_kode = '$sitKode'");
        if (!$checkco) {
            echo "{success:false, msg:\"Please insert CO first!\"}";
            return false;
        }
        ...
        foreach ($boq3 as $key => $val) {
            ...
            unset($boq3[$key]['invalid']);
            unset($boq3[$key]['invalid_msg']);
...
//                        }
                        ...
                        $jsonData2[0]['user_approval'] = $userApp;
                        $jsonData2[0]['ket'] = $tempOHbudgeth['ket'];
...
//				"statusfinal" => 'N',
        ...
        $this->regisco->insert($arrayInsert);
         $activityHead['transengineer_registerco'][0]=$arrayInsert;
...
//            $name = $name['displayname'][0];
            $name = QDC_User_Ldap::factory(array("uid" => $cek['assignto']))->getName();
            $cek['username'] = $name;
            $this->view->data = $cek;
            $this->view->trano = $trano;
        }
...
//        $name = $name['displayname'][0];
        $name =QDC_User_Ldap::factory(array("uid" => $data['data'][0]['assignto']))->getName();
...
//                $name = $name['displayname'][0];
                $name = QDC_User_Session::factory()->getCurrentName();
    ...
    public function editsiteboq3Action() {
        
    }

    public function editcfsboq3Action() {
        
    }

    public function cekcfsAction($boq3 = '') {
        if ($boq3 == '') {
            return false;
        }
        ...
        foreach ($boq3 as $k => $v) {
            ...
            $cfs_kode = $v['cfs_kode'];
            if ($cfs_kode == '') {
                $msg[] = "Error on Kode Barang : " . $v['kode_brg'] . " (Row $baris) : cfs kode is empty";
            }
            if (!is_numeric($cfs_kode)) {
                $msg[] = "Error on Kode Barang : " . $v['kode_brg'] . " (Row $baris) : cfs kode must be numeric character";
            }

            $cfs_nama = $v['cfs_nama'];
            if ($cfs_nama == '') {
                $msg[] = "Error on Kode Barang : " . $v['kode_brg'] . " (Row $baris) : cfs name is empty";
            }
        }
    ...
    public function doupdatecfsboq3Action() {
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout->disableLayout();

        $boq3 = Zend_Json::decode($this->getRequest()->getParam('boq3data'));
        $prj_kode = $this->getRequest()->getParam('prj_kode');
        $sit_kode = $this->getRequest()->getParam('sit_kode');

        $cek = $this->cekcfsAction($boq3);
        ...
        foreach ($boq3 as $key => $val) {
            ...
            $this->boq3->update($update, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' AND workid = '{$val['workid']}' AND kode_brg = '{$val['kode_brg']}'");
            $this->cboq3->update($update, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' AND workid = '{$val['workid']}' AND kode_brg = '{$val['kode_brg']}'");
        }

//        var_dump($boq3,$prj_kode,$sit_kode);
    ...
    public function doupdatesiteboq3Action() {
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout->disableLayout();

        $this->cboq3H = new Default_Models_MasterCboq3H();
        $this->counter = new Default_Models_MasterCounter();
        $this->master_site = new Default_Models_MasterSite();
        ...
        $this->prpg = new ProjectManagement_Models_ProjectProgress();

//        $boq3 = Zend_Json::decode($this->getRequest()->getParam('boq3data'));
        $prj_kode = $this->getRequest()->getParam('prj_kode');
        $sit_kode = $this->getRequest()->getParam('sit_kode');
        $new_sit_kode = $this->getRequest()->getParam('new_sit_kode');
        $new_sit_name = $this->getRequest()->getParam('new_sit_name');

        $ceksiteboq3 = $this->boq3->fetchAll("prj_kode = '$prj_kode' AND sit_kode = '$new_sit_kode'")->toArray();
        $ceksitecboq3 = $this->cboq3->fetchAll("prj_kode = '$prj_kode' AND sit_kode = '$new_sit_kode'")->toArray();

        $oldsitedata = $this->master_site->fetchRow("prj_kode = '$prj_kode' AND sit_kode = '$sit_kode'");

//        var_dump($oldsitedata);die;

        if ($ceksiteboq3) {
            $json = "{success: false, msg: 'Sorry, Site code has been exists'}";

            $this->getResponse()->setHeader('Content-Type', 'text/javascript');
            $this->getResponse()->setBody($json);

            return false;
        } else if ($ceksitecboq3) {
            $json = "{success: false, msg: 'Sorry, Site code has been exists'}";
            ...
            $updatesite = array(
                "sit_kode" => $new_sit_kode,
                "sit_nama" => $new_sit_name
            );

            $this->boq3->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->cboq3->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");

            $this->master_site->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");

            $this->boq3d->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->kboq3d->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->praboq3d->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->prd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->pod->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->ripd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->arfd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->asfd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->asfdcancel->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->remd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->dnd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->bpvd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->pay_arfd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->pay_rpid->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->pay_remd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->pay_dnd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->whod->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->pointd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->whbringbackd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->whreturnd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->whsupplierd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->afed->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->afesaving->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->praohpd->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->sald->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->piecemeal->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->dboq->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");
            $this->prpg->update($updatesite, "prj_kode = '$prj_kode' AND sit_kode = '$sit_kode' ");

            $type = 'SITE';
            $trano = $this->counter->setNewTrans($type);

            $log['site-before'] = array(
                "site_code" => $sit_kode,
                "site_name" => $oldsitedata['sit_nama']
            );

            $log2['site-after'] = array(
                "site_code" => $new_sit_kode,
                "site_name" => $new_sit_name
            );
    ...
    public function getBoq3Action()
    {
        ...
        $offset = ($this->_getParam("start") == '') ? 0 : $this->_getParam("start");
        $limit = ($this->_getParam("limit") == '') ? 100 : $this->_getParam("limit");
        $showAll = ($this->_getParam("all") == 'false') ? false : true;

        $boq3 = $this->budget->getBoq3All('all-current', $prjKode, $sitKode, $offset, $limit);
        $data = array();
        $total = 0;
        if ($boq3)
        {
            ...
            $select = $this->db->select()
                ->from(array("boq3_revisi"),array(
                    new Zend_Db_Expr("SQL_CALC_FOUND_ROWS *"),
                    "prj_kode" => new Zend_Db_Expr("'$prjKode'"),
                    "prj_nama" => new Zend_Db_Expr("'" . $p->getProjectName($prjKode) . "'"),
                    "sit_kode" => new Zend_Db_Expr("'$sitKode'"),
                    "sit_nama" => new Zend_Db_Expr("'" . $s->getSiteName($prjKode,$sitKode) . "'"),
                    "harga" => new Zend_Db_Expr("IF(val_kode != 'IDR', hargaUSD, hargaIDR)"),
                    "total" => new Zend_Db_Expr("IF(val_kode != 'IDR', (totalHargaUSD), (totalHargaIDR))")
                ))
                ->order(array("workid ASC"), array("kode_brg ASC"))
                ->limit($limit,$offset);

            if (!$showAll)
            {
                $select = $select->where("(totalHargaUSD) > 0 AND (totalHargaIDR) > 0");
            }
        ...
        $return = array("success" => true, "data" => $data, "total" => $total);
