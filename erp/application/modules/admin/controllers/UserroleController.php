...
class Admin_UserroleController extends Zend_Controller_Action {
    ...
    public function init() {
        ...
        $this->role = new Admin_Models_Masterrole();
    ...
    public function listuserroleAction() {
        ...
        if ($userID == '')
            return;
        $return = $this->userRole->getUserRoleByUserID($userID,$prj_kode);
        $json = Zend_Json::encode($return);
        $this->getResponse()->setHeader('Content-Type', 'text/javascript');
        $this->getResponse()->setBody($json);
    }
...
//                    $result[$key]['name'] = $account['displayname'][0];
                    $result[$key]['name'] = QDC_User_Ldap::factory(array("uid" => $val['uid']))->getName();
...
//                    $result[$key]['name'] = $account['displayname'][0];
    ...
    public function showLdapAction() {
        $this->_helper->viewRenderer->setNoRender();
        $uid = $this->request->getParam('uid');
        $ldap = new Default_Models_Ldap();

        $account = $ldap->getAccount($uid);
        var_dump($account);
    }
    ...
    public function getRoleAction() {
        ...
        $role_name = $this->request->getParam('role_name');
        $display_name = $this->request->getParam('display_name');

        $offset = ($this->_getParam("start") != '') ? $this->_getParam("start") : 0;
        $limit = ($this->_getParam("limit") != '') ? $this->_getParam("limit") : 20;

        $u = new Admin_Models_Masterroletype();

        $select = $this->db->select()
                ->from(array($u->__name()), array(
                    new Zend_Db_Expr("SQL_CALC_FOUND_ROWS *")
                ))
                ->order(array("role_name ASC", "display_name ASC"))
                ->limit($limit, $offset);

        if ($role_name) {
            $select = $select->where("role_name = ?", $role_name);
        }

        if ($display_name) {
            $select = $select->where("display_name = ?", $display_name);
        }
