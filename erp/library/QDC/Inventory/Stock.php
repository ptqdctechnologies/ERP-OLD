...
//
    ...
    public function createTableAllStock() {
        ...
        $query2 = "CREATE TEMPORARY TABLE all_stock
                  ( `id` INT NOT NULL AUTO_INCREMENT , 
                    `kode_brg` VARCHAR(45) NULL ,
                    `nama_brg` VARCHAR(250) NULL , 
                    `tgl` DATE DEFAULT '0000-00-00',
                    `sat_kode` VARCHAR(45) NULL , 
                    `gdg_nama` VARCHAR(45) NULL , 
                    `saldo_awal` DECIMAL(22,4) DEFAULT '0.0000' ,
                    `saldo_masuk` DECIMAL(22,4) DEFAULT '0.0000' ,
                    `saldo_keluar` DECIMAL(22,4) DEFAULT '0.0000' ,
                    `saldo_akhir` DECIMAL(22,4) DEFAULT '0.0000' ,
              PRIMARY KEY (`id`) ,
              UNIQUE INDEX `id_UNIQUE` (`id` ASC));
...
//        $fetch->execute();
        ...
        //Ambil data iSupp---------------------------------------------------------------------
        $isuppdata = $this->db->select()
                ->from(
                        array("a" => $this->LOGISTIC->LogisticInputSupplier->__name()), array(
                    "a.kode_brg",
                    "a.nama_brg",
                    "tgl" => new Zend_Db_Expr("MAX(a.tgl)"),
                    "b.sat_kode",
                    "a.gdg_kode",
                    "qty" => new Zend_Db_Expr($qty),
                    "status" => new Zend_Db_Expr("'SUP'")
                        )
                )->join(array("b" => $this->DEFAULT->MasterBarang->__name()), 
                        "a.kode_brg=b.kode_brg AND b.stspmeal ='N'", array())
                ->group(array("a.kode_brg", "a.gdg_kode"));
       
        //Ambil data i-Can
        $icandata = $this->db->select()
                ->from(
...
//                ->order(array("a.kode_brg"));
        ...
        //ambil saldo stock
        $accdata = $this->db->select()
                ->from(array("a" => $this->FINANCE->AccountingSaldoStock->__name()), array(
                    "a.kode_brg",
                    "a.nama_brg",
                    "tgl" => new Zend_Db_Expr("MAX(a.tgl)"),
                    "b.sat_kode",
...
//                        $bulan = '12';
//                        $tahun = (string) ($tahun - 1);
//                        $accdata = $accdata->where("periode = '$bulan' AND tahun = '$tahun'");
//                    } else {
//                        $tahun = (string) ($tahun);
//
//                        if ($bulan <= 10)
...
//        }
        ...
        $select = $this->db->select()
                    ->union(array($isuppdata, $icandata, $ilovdata, $dodata, $accdata));
        ...
        $select2 = $this->db->select()
                ->from(
                        array("a" => $select), array(
                            ...
                            "a.tgl", 
                            "a.sat_kode",
                            "gdg_nama" => new Zend_Db_Expr("IFNULL(b.gdg_nama, 'Gudang tidak ditemukan')"),
                            "saldo_awal" => new Zend_Db_Expr("sum(CASE WHEN YEAR(a.tgl) < '".$periode."' THEN a.qty ELSE 0 END)"),
                            "saldo_masuk" => new Zend_Db_Expr("sum(CASE WHEN YEAR(a.tgl) = '".$periode."' AND a.status != 'DO' THEN a.qty ELSE 0 END)"),
                            "saldo_keluar" => new Zend_Db_Expr("sum(CASE WHEN YEAR(a.tgl) = '".$periode."' AND a.status = 'DO' THEN a.qty ELSE 0 END)"),
                            "saldo_akhir" => new Zend_Db_Expr("sum(CASE WHEN YEAR(a.tgl) < '".$periode."' THEN a.qty ELSE 0 END) + 
                                            (sum(CASE WHEN YEAR(a.tgl) = '".$periode."' AND a.status != 'DO' THEN a.qty ELSE 0 END) + 
                                            sum(CASE WHEN YEAR(a.tgl) = '".$periode."' AND a.status = 'DO' THEN a.qty ELSE 0 END))"),
                                )
                )->joinLeft(array("b" => $this->LOGISTIC->MasterGudang->__name()), 
                        "a.gdg_kode=b.gdg_kode", array())
                ->group(array("a.kode_brg", "a.gdg_kode"))
                ->order(array("a.kode_brg ASC"));
        
        if ($kode_brg)
            $select2 = $select2->where("a.kode_brg=?", $kode_brg);

        if ($gdg_kode)
            $select2 = $select2->where("a.gdg_kode=?", $gdg_kode);
        
        if ($periode)
            $select2 = $select2->where("YEAR(a.tgl) <=?", $periode);
        
        $selectSql = (string) $select2; 
        $selectSql = "
            INSERT INTO all_stock
            $selectSql
        ";
        
        $fetch = $this->db->prepare($selectSql);
        $fetch->execute();
