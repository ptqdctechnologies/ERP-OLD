...
Class QDC_Jasper_Report
{
    ...
    public function compilerSubReport($filePath)
    {
        $r = $this->JASPER->compileManager->compileReport($filePath);

        return $r;
    }

    private function fillerSubReport($compiledReport='',$arrayData,$arrayParams)
    {
        if ($this->dataSource == NO_DATA_SOURCE)
        {
            $data = $this->arrayToJavaArrayList($arrayData);
            $ds = new java("net.sf.jasperreports.engine.data.JRBeanCollectionDataSource",$data);
            $filled = $this->JASPER->fillManager->fillReport($compiledReport, $arrayParams,$ds);
        }
        else
        {
            $filled = $this->JASPER->fillManager->fillReport($compiledReport, $arrayParams,$this->JASPER->dbConn->getConnection());
        }
