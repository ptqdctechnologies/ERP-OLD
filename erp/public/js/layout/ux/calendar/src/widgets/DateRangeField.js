...
Ext.calendar.DateRangeField = Ext.extend(Ext.form.Field, {
    ...
    checkDates: function(type, startend) {
        ...
        if (type == 'time')
        {
            ...
            {
                Ext.Msg.alert("Error","You cannot add event on Lunch Time!");
                startField.setValue(startValue.add(Date.HOUR,-1));
                endField.setValue(endValue.add(Date.HOUR,-1));
                return false;
            }
            if (startValue.getHours() ==  endValue.getHours())
            {
                if (startValue.getHours() != 17)
                    endField.setValue(endValue.add(Date.HOUR,+1));
                else
                    startField.setValue(startValue.add(Date.HOUR,-1));
            }
    ...
    getDT: function(startend) {
        ...
        //Gak tau kenapa isVisible() selalu false padahal komponen endTime gak hidden....
